...
#define FORCE_POWER_DELAY_DEFAULT 18000 /* millisecond */
...
static struct long_press_work power_button_work = {
	...
	.force_power_delay_ms = FORCE_POWER_DELAY_DEFAULT,
};
	...
	if (type == EV_KEY && code == KEY_POWER) {
		...
		if (value && !power_button_work.is_pressed) {
			...
			schedule_delayed_work(&power_button_work.dwork,
				msecs_to_jiffies(power_button_work.force_power_delay_ms));
...
static int power_keydebug_probe(struct platform_device *pdev)
{
	...
	u32 delay = FORCE_POWER_DELAY_DEFAULT;

	pr_info("Power key debug probe\n");

	if (np)
		of_property_read_u32(np, "google,force-power-delay-ms", &delay);
	power_button_work.force_power_delay_ms = delay;
	pr_info("Power key debug force-power-delay-ms: %u\n", delay);
