				...
				unsigned int num_groups)
{
	...
	*pinctl_de = debugfs_create_dir(dev_name(gctl->pctl->dev), NULL);
	...
	debugfs_create_bool("resume-dump", 0600, *pinctl_de, &gctl->resume_dump_enabled);
	tmp = debugfs_lookup("resume-dump", *pinctl_de);
	if (IS_ERR_OR_NULL(tmp)) {
		ret = PTR_ERR(tmp);
		dev_err(gctl->dev, "Failed to create resume-dump: %d\n", ret);
	} else {
		++success_cnt;
		dput(tmp);
	}

	debugfs_create_bool("gpio-set-check", 0600, *pinctl_de, &gctl->set_check_enabled);
	tmp = debugfs_lookup("gpio-set-check", *pinctl_de);
	if (IS_ERR_OR_NULL(tmp)) {
		ret = PTR_ERR(tmp);
		dev_err(gctl->dev, "Failed to create gpio-set-check: %d\n", ret);
	} else {
		++success_cnt;
		dput(tmp);
	}
