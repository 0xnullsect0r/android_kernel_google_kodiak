				    ...
				    loff_t *ppos)
{
	...
	} else if (strncmp(buf, "enable_wake", 11) == 0) {
		ret = enable_irq_wake(trigger_data->irq);
	} else if (strncmp(buf, "disable_wake", 12) == 0) {
...
static void gia_test_remove(struct platform_device *pdev)
{
