...
#define ASSERT_STR(assert) ((assert) ? "assert" : "deassert")
...
/*
 * See b/537323729. The peri clocks are virtual groups of resets that require
 * a specific sequence of asserting/deasserting the IP clk and IP cfg_clk.
 */
static int goog_cpm_rst_peri_sequence(struct goog_cpm_rst *goog_cpm_rst, unsigned long id,
				      const struct cpm_peri_rst_map *peri_map, bool assert)
{
	struct device *dev = goog_cpm_rst->rcdev.dev;
	int op_id = assert ? ASSERT_OP_ID : DEASSERT_OP_ID;
	int ret;

	dev_dbg(dev, "intercepted %s %lu for peri clock sequencing\n", ASSERT_STR(assert), id);
	ret = goog_cpm_rst_send_mba_mail(goog_cpm_rst, peri_map->cfg_clk_id, op_id);
	if (ret) {
		dev_err(dev, "lpcm_id %u, rst_id %lu: failed to %s cfg_clk %lu, ret=%d\n",
			goog_cpm_rst->lpcm_id, id, ASSERT_STR(assert), peri_map->cfg_clk_id, ret);
		return ret;
	}

	/*
	 * A busy-wait (ndelay) is used because HPG dictates this exact
	 * duration. Using a sleeping function is impractical as arming a timer
	 * would introduce excessive overhead.
	 */
	if (!assert)
		ndelay(200);

	ret = goog_cpm_rst_send_mba_mail(goog_cpm_rst, peri_map->clk_id, op_id);
	if (ret) {
		dev_err(dev, "lpcm_id %u, rst_id %lu: failed to %s clk %lu, state is partially %sed, ret=%d\n",
			goog_cpm_rst->lpcm_id, id, ASSERT_STR(assert), peri_map->clk_id,
			ASSERT_STR(assert), ret);
		return ret;
	}
			       ...
			       unsigned long id)
{
	...
	const struct cpm_peri_rst_map *peri_map;

	dev_dbg(rcdev->dev, "assert reset id=%lu of lpcm %u\n", id,
		goog_cpm_rst->lpcm_id);

	peri_map = goog_cpm_rst_find_peri_map(goog_cpm_rst->lpcm_id, id);
	if (peri_map)
		return goog_cpm_rst_peri_sequence(goog_cpm_rst, id, peri_map, true);
				 ...
				 unsigned long id)
{
	...
	peri_map = goog_cpm_rst_find_peri_map(goog_cpm_rst->lpcm_id, id);
	if (peri_map)
		return goog_cpm_rst_peri_sequence(goog_cpm_rst, id, peri_map, false);
...
static void goog_cpm_rst_remove(struct platform_device *pdev)
{
