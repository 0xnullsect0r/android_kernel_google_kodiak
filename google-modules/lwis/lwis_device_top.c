								       ...
								       int64_t trigger_event_id)
{
	struct lwis_top_device *lwis_top_dev =
		container_of(lwis_dev, struct lwis_top_device, base_dev);
	struct lwis_event_subscriber_list *event_subscriber_list = lwis_allocator_allocate(
		lwis_dev, sizeof(struct lwis_event_subscriber_list), GFP_ATOMIC);
				    ...
				    int trigger_device_id, int subscriber_device_id)
{
	...
	new_subscription = lwis_allocator_allocate(
		lwis_dev, sizeof(struct lwis_event_subscribe_info), GFP_ATOMIC);
	if (!new_subscription) {
		spin_unlock_irqrestore(&lwis_top_dev->base_dev.lock, flags);
		return -ENOMEM;
	}
			...
			if (list_empty(&subscribe_info->event_subscriber_list->list)) {
				...
				list_for_each_entry_safe(pending_event, n,
					...
					if (pending_event->trigger_event_id == trigger_event_id) {
						...
						lwis_allocator_free(lwis_dev, pending_event);
					}
				}
...
static void top_event_subscribe_clear(struct lwis_top_device *lwis_top_dev)
{
	...
	int i;
	unsigned long flags;
	struct list_head subscriptions_to_clear;
	...
	list_for_each_entry_safe(pending_event, n, &lwis_top_dev->emitted_event_list_work, node) {
		list_del(&pending_event->node);
		lwis_allocator_free(&lwis_top_dev->base_dev, pending_event->payload);
		lwis_allocator_free(&lwis_top_dev->base_dev, pending_event);
	}
	spin_unlock_irqrestore(&lwis_top_dev->base_dev.lock, flags);
	...
	list_for_each_safe(it_event_subscriber, it_event_subscriber_tmp, &subscriptions_to_clear) {
		...
		lwis_allocator_free(&lwis_top_dev->base_dev, subscribe_info);
	}
