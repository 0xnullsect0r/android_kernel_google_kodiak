...
struct lwis_transaction_info_v7 {
	// Input
	int64_t trigger_event_id;
	int64_t trigger_event_counter;
	struct lwis_transaction_trigger_condition trigger_condition;
...
struct lwis_transaction_info {
	...
	// If true, we will skip each lwis_device_io_entry where the corresponding device is
	// powered off.
	bool skip_device_io_entries_if_powered_off;
	bool run_in_event_context;
	// Use reserved to keep the original interface
	bool reserved;
	int64_t emit_success_event_id;
...
struct lwis_dpm_op_level {
	// read back value from system to be sent back to the user.
	uint64_t level;
	// requesting entity for the op level readback.
	char entity[LWIS_MAX_NAME_STRING_LEN];
};
...
enum lwis_cmd_id {
	...
	LWIS_CMD_ID_TRANSACTION_SUBMIT_V7,
	...
	LWIS_CMD_ID_DPM_OP_LEVEL_GET = 0x70300,

	LWIS_CMD_ID_FENCE_CREATE = 0x80001,
	LWIS_CMD_ID_RELEASE_FENCE_CREATE = 0x80100,
...
struct lwis_cmd_transaction_info_v7 {
	struct lwis_cmd_pkt header;
	struct lwis_transaction_info_v7 info;
};
...
struct lwis_cmd_dpm_op_level_get {
	struct lwis_cmd_pkt header;
	struct lwis_dpm_op_level reqs;
};
