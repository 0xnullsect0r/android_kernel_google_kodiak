...
enum mtk_cldma_err_event {
	CLDMA_NOT_ACCESSIBLE = 0,
};
...
struct txq {
	struct cldma_drv_info *drv_info;
	struct mutex tx_mtx; /* Protecting tx flow */
	u32 txqno;
	struct queue_info *que;
...
struct mtk_ctrl_drv_cfg {
	void (*tx_done_work)(struct work_struct *work);
	void (*rx_done_work)(struct work_struct *work);
};
