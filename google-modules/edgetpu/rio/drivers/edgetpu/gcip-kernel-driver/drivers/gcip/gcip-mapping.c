							    ...
							    struct sg_table **sgt_default_ptr)
{
	 ...
	 * exporter then we don't need CMOs for our client context mapping here.  If SKIP_CPU_SYNC
	 * isn't set then the associated CMOs were performed for the default domain mapping by the
	 * dma-buf exporter during the dma_buf_map_attachment call; we don't need a second sync
	 * here.  We make a copy of the gcip_map_flags to avoid modifying the flags reported via
	 * debugfs, which could be confusing.
	 */
