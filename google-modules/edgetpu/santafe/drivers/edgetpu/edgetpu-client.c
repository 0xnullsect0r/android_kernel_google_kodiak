...
void edgetpu_client_log_state(struct edgetpu_client *client)
{
	...
	pasid = (etdomain && !edgetpu_mmu_domain_detached(etdomain)) ? etdomain->pasid : -1;
	etdev_info(etdev, "vcid=%u pasid=%d stat=%u err=%#x cmdpend=%d cancel=%d", group->vcid,
		   pasid, group->status, group->fatal_errors,
