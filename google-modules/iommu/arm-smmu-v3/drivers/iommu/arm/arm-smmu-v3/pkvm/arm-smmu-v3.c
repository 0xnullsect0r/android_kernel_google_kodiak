// SPDX-License-Identifier: GPL-2.0
/*
 * pKVM hyp driver for the Arm SMMUv3
 *
 * Copyright (C) 2022 Linaro Ltd.
 */
#include <arm-smmu-v3/arm-smmu-v3-common.h>
#include <asm/kvm_hyp.h>
#include <asm/kvm_pkvm_module.h>
#include <arm-smmu-v3/io-pgtable-arm.h>
#include <nvhe/alloc.h>
#include <nvhe/iommu.h>
#include <nvhe/mem_protect.h>
#include <nvhe/mm.h>
#include <nvhe/pkvm.h>
#include <nvhe/rwlock.h>
#include <nvhe/trap_handler.h>

#include "arm-smmu-v3/arm-smmu-v3-telemetry-callbacks.h"
#include "arm_smmu_v3.h"
#include "arm-smmu-v3-module.h"
#include "hyp-arm-smmu-v3-common-telemetry.h"
#include "hyp-arm-smmu-v3-telemetry.h"

#ifdef MODULE
void *memset(void *dst, int c, size_t count)
{
	return CALL_FROM_OPS(memset, dst, c, count);
}

void *memcpy(void *dst, const void *src, size_t count)
{
	return CALL_FROM_OPS(memcpy, dst, src, count);
}

#ifdef CONFIG_LIST_HARDENED
bool __list_add_valid_or_report(struct list_head *new,
				struct list_head *prev,
				struct list_head *next)
{
	return CALL_FROM_OPS(list_add_valid_or_report, new, prev, next);
}

bool __list_del_entry_valid_or_report(struct list_head *entry)
{
	return CALL_FROM_OPS(list_del_entry_valid_or_report, entry);
}
#endif

const struct pkvm_module_ops		*mod_ops;
#endif

#define ARM_SMMU_POLL_TIMEOUT_US	100000 /* 100ms arbitrary timeout */
/* Firmware page size is 4 KB and independent of Linux page size. */
#define FIRMWARE_PAGE_SHIFT		12
#define SMC_FC_OEM_SET_NS_IPA_TBL	0x8300000E
#define SET_NS_IPA_CFG_GRANULE		GENMASK(9, 8)
#define SET_NS_IPA_CFG_START_LVL	GENMASK(7, 6)
#define SET_NS_IPA_CFG_T0SZ		GENMASK(5, 0)
#define SET_NS_IPA_CFG_COHERENT_WALK	BIT(10)
#define SMC_FC_OEM_INV_NS_IPA_START	0x8300000F
#define SMC_FC_OEM_INV_NS_IPA_COMPLETE	0x83000010
#define SMC_FC_OEM_NS_SMMU_STATE	0x83000011

size_t __ro_after_init kvm_hyp_arm_smmu_v3_count;
struct hyp_arm_smmu_v3_device *kvm_hyp_arm_smmu_v3_smmus;
struct hyp_arm_smmu_v3_err *kvm_hyp_smmu_last_err;
struct hyp_arm_smmu_v3_global_config kvm_hyp_smmu_global_config;

/*
 * Wait until @cond is true.
 * Return 0 on success, or -ETIMEDOUT
 */
#define smmu_wait(_cond)					\
({								\
	int __i = 0;						\
	int __ret = 0;						\
								\
	while (!(_cond)) {					\
		if (++__i > ARM_SMMU_POLL_TIMEOUT_US) {		\
			__ret = -ETIMEDOUT;			\
			break;					\
		}						\
		pkvm_udelay(1);					\
	}							\
	__ret;							\
})

/* Timer freq in HZ */
static u32 arch_timer_rate;
static u64 smmu_poll_timeout_cycle;

/*
 * Use WFET if possible, otherwise, rely on WFE and eventstream
 * should wake us up.
 */
#define smmu_wfe(end)							\
({									\
	if (alternative_has_cap_unlikely(ARM64_HAS_WFXT))		\
		wfet(end);						\
	else								\
		wfe();							\
})

#define smmu_wait_wfe(_cond)						\
({									\
	u64 __start_cnt = __arch_counter_get_cntvct();			\
	u64 __end_cnt = __start_cnt + smmu_poll_timeout_cycle;		\
	int __ret = 0;							\
									\
	while (!(_cond)) {						\
		smmu_wfe(__end_cnt);					\
		if ((__arch_counter_get_cntvct() - __start_cnt) >=	\
		    smmu_poll_timeout_cycle) {				\
			__ret = -ETIMEDOUT;				\
			break;						\
		}							\
	}								\
	__ret;								\
})

#define smmu_wait_event(_smmu, _cond)				\
({								\
	int __ret;						\
	if ((_smmu)->features & ARM_SMMU_FEAT_SEV) {		\
		__ret = smmu_wait_wfe(_cond);			\
	} else {						\
		__ret = smmu_wait(_cond);			\
	}							\
	__ret;							\
})

struct domain_iommu_node {
	struct kvm_hyp_iommu		*iommu;
	struct list_head		list;
	unsigned long			ref;
};

/*
 * SMMUv3 domain:
 * @domain: Pointer to the IOMMU domain.
 * @iommu_list: List of SMMU instances for this domain
 * @list_lock: Protects iommu_list and the entry member for nested domains.
 * @type: Type of domain (S1, S2)
 * @pgt_lock: Lock for page table
 * @pgtable: io_pgtable instance for this domain
 * @cdptr: shared cdtable for this domain
 * @pgsize_bitmap: The supported page-size bitmap for this domain
 * @vmid: the VMID associated with this domain
 * @num_nested_s1: number of streams using this S2 table with an S1 table
 */
struct hyp_arm_smmu_v3_domain {
	struct kvm_hyp_iommu_domain     *domain;
	struct list_head		iommu_list;
	hyp_rwlock_t			list_lock;
	u32				type;
	hyp_spinlock_t			pgt_lock;
	struct io_pgtable		*pgtable;
	phys_addr_t			cdptr;
	unsigned long			pgsize_bitmap;
	u16				vmid;
	unsigned long			num_nested_s1;
	u64				pgt_version;
	bool				snapshot_done;
};

/*
 * SMMUv3 nested domain:
 * @smmu_domain: The base information shared across all SMMUv3 types of domains.
 */
struct hyp_arm_smmu_v3_nested_domain {
	struct hyp_arm_smmu_v3_domain	smmu_domain;
};

/* IOMMU telemetry APIs */
static enum pgsize_idx get_pgsize_idx(size_t size)
{
	switch (size) {
	case SZ_4K:   return IDX_4K;
	case SZ_16K:  return IDX_16K;
	case SZ_64K:  return IDX_64K;
	case SZ_2M:   return IDX_2M;
	case SZ_32M:  return IDX_32M;
	case SZ_512M: return IDX_512M;
	case SZ_1G:   return IDX_1G;
	case SZ_16G:  return IDX_16G;
	default:      return IDX_MAX;
	}
}

static void sz_stats_to_map_counters(struct arm_lpae_io_pgtable *data,
				     struct sz_stats *sz_stats,
				     struct map_counters_by_page_size *dest)
{
	int lvl;
	size_t sz, cont_sz;
	int adj_cont;
	enum pgsize_idx idx;

	for (lvl = data->start_level; lvl < ARM_LPAE_MAX_LEVELS; lvl++) {
		sz = ARM_LPAE_BLOCK_SIZE(lvl, data);
		adj_cont = arm_lpae_adjacent_contptes(lvl, data);
		cont_sz = sz * adj_cont;

		if (sz_stats->non_cont[lvl]) {
			idx = get_pgsize_idx(sz);
			if (idx < IDX_MAX)
				dest->counters[idx] += sz_stats->non_cont[lvl];
		}

		if (sz_stats->cont[lvl]) {
			idx = get_pgsize_idx(cont_sz);
			if (idx < IDX_MAX)
				dest->counters[idx] += sz_stats->cont[lvl];
		}
	}

	dest->pgtables_used += sz_stats->pgtables_used;
	dest->empty_pgtables += sz_stats->empty_pgtables;
}

static struct hyp_arm_smmu_v3_domain *smmu_domains[MAX_SMMU_DOMAIN];
static hyp_spinlock_t smmu_domains_lock = __HYP_SPIN_LOCK_UNLOCKED;

static int domain_get(struct kvm_hyp_iommu_domain *domain)
{
	int old;

	if (domain) {
		old = __ll_sc_atomic_fetch_add_acquire(1, &domain->refs);
		BUG_ON(!old || (old + 1 < 0));
	}

	return 0;
}

static void domain_put(struct kvm_hyp_iommu_domain *domain)
{
	if (domain)
		BUG_ON(!__ll_sc_atomic_sub_return_release(1, &domain->refs));
}

static struct hyp_arm_smmu_v3_domain *smmu_id_to_domain(pkvm_handle_t domain_id)
{
	struct hyp_arm_smmu_v3_domain *smmu_domain;

	if (domain_id >= MAX_SMMU_DOMAIN)
		return NULL;

	domain_id = array_index_nospec(domain_id, MAX_SMMU_DOMAIN);
	hyp_spin_lock(&smmu_domains_lock);
	smmu_domain = smmu_domains[domain_id];
	if (smmu_domain)
		domain_get(smmu_domain->domain);
	hyp_spin_unlock(&smmu_domains_lock);

	return smmu_domain;
}

void pkvm_ptw_hvc_call(struct user_pt_regs *regs)
{
	pkvm_handle_t domain_id = (pkvm_handle_t)regs->regs[PTW_ARG_DOMAIN_ID];
	unsigned long iova_start = (unsigned long)regs->regs[PTW_ARG_IOVA_START];
	struct hyp_arm_smmu_v3_domain *smmu_domain;
	struct io_pgtable *pgtable;
	struct map_counters_by_page_size *dest;
	struct sz_stats stack_stats = {0};
	u64 start_time, end_time, ns;
	u32 freq = arch_timer_rate;

	smmu_domain = smmu_id_to_domain(domain_id);
	if (!smmu_domain) {
		regs->regs[PTW_RET_STATUS_HVC] = 0;
		regs->regs[PTW_RET_STATUS_WALK] = PTW_ERROR_NODEV;
		trace_hyp_printk("SMMU domain not found for domain %u", domain_id);
		return;
	}

	pgtable = smmu_domain->pgtable;
	if (!pgtable || !pgtable->ops.audit) {
		regs->regs[PTW_RET_STATUS_HVC] = 0;
		regs->regs[PTW_RET_STATUS_WALK] = PTW_ERROR_INVAL;
		trace_hyp_printk("SMMU domain %d doesn't have pgtable or audit ops", domain_id);
		domain_put(smmu_domain->domain);
		return;
	}

	if (domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID) {
		dest = &kvm_hyp_shared_arm_smmu_telemetry->hs2t.s2_map_counters;
	} else if (domain_id > 0 && domain_id < MAX_SMMU_DOMAIN) {
		struct hyp_arm_smmu_domain_telemetry *hasdt;

		hasdt = domain_id_to_hasdt(domain_id);
		if (!hasdt) {
			regs->regs[PTW_RET_STATUS_HVC] = 0;
			regs->regs[PTW_RET_STATUS_WALK] = PTW_ERROR_INVAL;
			domain_put(smmu_domain->domain);
			return;
		}
		dest = &hasdt->map_counters;
	} else {
		regs->regs[PTW_RET_STATUS_HVC] = 0;
		regs->regs[PTW_RET_STATUS_WALK] = PTW_ERROR_INVAL;
		domain_put(smmu_domain->domain);
		return;
	}

	hyp_spin_lock(&smmu_domain->pgt_lock);
	if (iova_start == 0) {
		if ((dest->pgt_version & PTW_VERSION_MASK) ==
		    (smmu_domain->pgt_version & PTW_VERSION_MASK)) {
			hyp_spin_unlock(&smmu_domain->pgt_lock);
			trace_hyp_printk("Domain %d up to date, version %llu",
					 domain_id, dest->pgt_version & PTW_VERSION_MASK);
			regs->regs[PTW_RET_STATUS_HVC] = 0;
			regs->regs[PTW_RET_IOVA_NEXT] = 0;
			regs->regs[PTW_RET_STATUS_WALK] = PTW_COMPLETE;
			domain_put(smmu_domain->domain);
			return;
		}
		/* New walk needed, clear counters and store version with in-progress bit */
		dest->pgtables_used = 0;
		dest->empty_pgtables = 0;
		memset(dest->counters, 0, sizeof(dest->counters));
		dest->pgt_version = (smmu_domain->pgt_version & PTW_VERSION_MASK) |
				    PTW_VERSION_INPROGRESS_BIT;
	} else {
		/*
		 * If the version changed while we were walking,
		 * restart from the beginning.
		 */
		if ((dest->pgt_version & PTW_VERSION_MASK) !=
		    (smmu_domain->pgt_version & PTW_VERSION_MASK)) {
			hyp_spin_unlock(&smmu_domain->pgt_lock);
			regs->regs[PTW_RET_STATUS_HVC] = 0;
			regs->regs[PTW_RET_STATUS_WALK] = PTW_RETRY;
			domain_put(smmu_domain->domain);
			return;
		}
	}

	start_time = __arch_counter_get_cntvct();
	pgtable->ops.audit(&pgtable->ops, iova_start, &stack_stats);
	end_time = __arch_counter_get_cntvct();

	sz_stats_to_map_counters(io_pgtable_to_data(pgtable), &stack_stats, dest);

	regs->regs[PTW_RET_STATUS_HVC] = 0;
	if (stack_stats.next_iova) {
		regs->regs[PTW_RET_IOVA_NEXT] = stack_stats.next_iova;
		regs->regs[PTW_RET_STATUS_WALK] = PTW_INCOMPLETE;
		/* Version remains with INPROGRESS_BIT set */
	} else {
		regs->regs[PTW_RET_IOVA_NEXT] = 0;
		regs->regs[PTW_RET_STATUS_WALK] = PTW_COMPLETE;
		/* Mark as finished (clear bit 63) with final version */
		dest->pgt_version = smmu_domain->pgt_version & PTW_VERSION_MASK;
	}
	hyp_spin_unlock(&smmu_domain->pgt_lock);

	ns = ((end_time - start_time) * 1000000000ULL) / freq;
	trace_hyp_printk("PTW walk for domain %d iova_start 0x%lx took %llu ns",
			 domain_id, iova_start, ns);
	trace_hyp_printk("PTW status %d, version %llu and iova 0x%lx",
			 (int)regs->regs[PTW_RET_STATUS_WALK],
			 dest->pgt_version & PTW_VERSION_MASK, stack_stats.next_iova);
	domain_put(smmu_domain->domain);
}
/* End of IOMMU telemetry code */

static struct hyp_arm_smmu_v3_device *to_smmu(struct kvm_hyp_iommu *iommu)
{
	return container_of(iommu, struct hyp_arm_smmu_v3_device, iommu);
}

static struct hyp_arm_smmu_v3_nested_domain *to_nested_domain(struct hyp_arm_smmu_v3_domain *dom)
{
	return container_of(dom, struct hyp_arm_smmu_v3_nested_domain, smmu_domain);
}

static int smmu_write_cr0(struct hyp_arm_smmu_v3_device *smmu, u32 val)
{
	writel_relaxed(val, smmu->base + ARM_SMMU_CR0);
	return smmu_wait(readl_relaxed(smmu->base + ARM_SMMU_CR0ACK) == val);
}

/* Transfer ownership of structures from host to hyp */
static int smmu_take_pages(u64 phys, size_t size)
{
	WARN_ON(!PAGE_ALIGNED(phys) || !PAGE_ALIGNED(size));
	return __pkvm_host_donate_hyp(phys >> PAGE_SHIFT, size >> PAGE_SHIFT);
}

static void smmu_reclaim_pages(u64 phys, size_t size)
{
	WARN_ON(!PAGE_ALIGNED(phys) || !PAGE_ALIGNED(size));
	WARN_ON(__pkvm_hyp_donate_host(phys >> PAGE_SHIFT, size >> PAGE_SHIFT));
}

#define Q_WRAP(smmu, reg)	((reg) & (1 << (smmu)->cmdq_log2size))
#define Q_IDX(smmu, reg)	((reg) & ((1 << (smmu)->cmdq_log2size) - 1))

static bool smmu_cmdq_has_space(struct hyp_arm_smmu_v3_device *smmu, u32 n)
{
	u64 smmu_cons = readl_relaxed(smmu->base + ARM_SMMU_CMDQ_CONS);
	u32 space, prod, cons;

	prod = Q_IDX(smmu, smmu->cmdq_prod);
	cons = Q_IDX(smmu, smmu_cons);

	if (Q_WRAP(smmu, smmu->cmdq_prod) == Q_WRAP(smmu, smmu_cons))
		space = (1 << smmu->cmdq_log2size) - (prod - cons);
	else
		space = cons - prod;

	return space >= n;
}

static bool smmu_cmdq_empty(struct hyp_arm_smmu_v3_device *smmu)
{
	u64 cons = readl_relaxed(smmu->base + ARM_SMMU_CMDQ_CONS);

	return Q_IDX(smmu, smmu->cmdq_prod) == Q_IDX(smmu, cons) &&
	       Q_WRAP(smmu, smmu->cmdq_prod) == Q_WRAP(smmu, cons);
}

static int smmu_build_cmd(u64 *cmd, struct arm_smmu_cmdq_ent *ent)
{
	memset(cmd, 0, CMDQ_ENT_DWORDS << 3);
	cmd[0] |= FIELD_PREP(CMDQ_0_OP, ent->opcode);

	switch (ent->opcode) {
	case CMDQ_OP_CFGI_STE_RANGE:
		cmd[0] |= FIELD_PREP(CMDQ_CFGI_0_SID, ent->cfgi.sid);
		cmd[1] |= FIELD_PREP(CMDQ_CFGI_1_RANGE, ent->cfgi.range);
		break;
	case CMDQ_OP_CFGI_CD:
		cmd[0] |= FIELD_PREP(CMDQ_CFGI_0_SSID, ent->cfgi.ssid);
		fallthrough;
	case CMDQ_OP_CFGI_STE:
		cmd[0] |= FIELD_PREP(CMDQ_CFGI_0_SID, ent->cfgi.sid);
		cmd[1] |= FIELD_PREP(CMDQ_CFGI_1_LEAF, ent->cfgi.leaf);
		break;
	case CMDQ_OP_TLBI_NH_VA:
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_VMID, ent->tlbi.vmid);
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_NUM, ent->tlbi.num);
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_SCALE, ent->tlbi.scale);
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_ASID, ent->tlbi.asid);
		cmd[1] |= FIELD_PREP(CMDQ_TLBI_1_LEAF, ent->tlbi.leaf);
		cmd[1] |= FIELD_PREP(CMDQ_TLBI_1_TTL, ent->tlbi.ttl);
		cmd[1] |= FIELD_PREP(CMDQ_TLBI_1_TG, ent->tlbi.tg);
		cmd[1] |= ent->tlbi.addr & CMDQ_TLBI_1_VA_MASK;
		break;
	case CMDQ_OP_TLBI_NSNH_ALL:
		break;
	case CMDQ_OP_TLBI_NH_ASID:
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_ASID, ent->tlbi.asid);
		fallthrough;
	case CMDQ_OP_TLBI_NH_ALL:
	case CMDQ_OP_TLBI_S12_VMALL:
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_VMID, ent->tlbi.vmid);
		break;
	case CMDQ_OP_TLBI_S2_IPA:
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_NUM, ent->tlbi.num);
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_SCALE, ent->tlbi.scale);
		cmd[0] |= FIELD_PREP(CMDQ_TLBI_0_VMID, ent->tlbi.vmid);
		cmd[1] |= FIELD_PREP(CMDQ_TLBI_1_LEAF, ent->tlbi.leaf);
		cmd[1] |= FIELD_PREP(CMDQ_TLBI_1_TTL, ent->tlbi.ttl);
		cmd[1] |= FIELD_PREP(CMDQ_TLBI_1_TG, ent->tlbi.tg);
		cmd[1] |= ent->tlbi.addr & CMDQ_TLBI_1_IPA_MASK;
		break;
	case CMDQ_OP_CMD_SYNC:
		cmd[0] |= FIELD_PREP(CMDQ_SYNC_0_CS, CMDQ_SYNC_0_CS_SEV);
		break;
	default:
		return -EINVAL;
	}

	return 0;
}

static int smmu_issue_cmds(struct hyp_arm_smmu_v3_device *smmu,
			   u64 *cmds, int n)
{
	int i;
	int ret;
	u32 prod;

	ret = smmu_wait_event(smmu, smmu_cmdq_has_space(smmu, n));
	if (ret)
		return ret;

	for (i = 0; i < n; i++) {
		int j;
		int idx = Q_IDX(smmu, smmu->cmdq_prod + i);
		u64 *slot = smmu->cmdq_base + idx * CMDQ_ENT_DWORDS;

		for (j = 0; j < CMDQ_ENT_DWORDS; j++)
			slot[j] = cpu_to_le64(cmds[i * CMDQ_ENT_DWORDS + j]);
	}

	prod = (Q_WRAP(smmu, smmu->cmdq_prod) | Q_IDX(smmu, smmu->cmdq_prod)) + n;
	smmu->cmdq_prod = Q_OVF(smmu->cmdq_prod) | Q_WRAP(smmu, prod) | Q_IDX(smmu, prod);

	writel(Q_IDX(smmu, smmu->cmdq_prod) | Q_WRAP(smmu, smmu->cmdq_prod),
	       smmu->base + ARM_SMMU_CMDQ_PROD);
	return 0;
}

static int smmu_add_cmd(struct hyp_arm_smmu_v3_device *smmu,
			struct arm_smmu_cmdq_ent *ent)
{
	u64 cmd[CMDQ_ENT_DWORDS] = {};
	int ret;

	ret = smmu_build_cmd(cmd, ent);
	if (ret)
		return ret;

	return smmu_issue_cmds(smmu, cmd, 1);
}

static int smmu_sync_cmd(struct hyp_arm_smmu_v3_device *smmu)
{
	int ret;
	u64 cmdq_sync_start_time;
	struct arm_smmu_cmdq_ent cmd = {
		.opcode = CMDQ_OP_CMD_SYNC,
	};

	cmdq_sync_start_time = __arch_counter_get_cntvct();
	ret = smmu_add_cmd(smmu, &cmd);
	if (ret)
		return ret;

	ret = smmu_wait_event(smmu, smmu_cmdq_empty(smmu));
	smmu_telemetry_cmdq_sync_latency(smmu,
					 (__arch_counter_get_cntvct() - cmdq_sync_start_time));

	return ret;
}

static int smmu_send_cmd(struct hyp_arm_smmu_v3_device *smmu,
			 struct arm_smmu_cmdq_ent *cmd)
{
	int ret;

	if (smmu->iommu.power_is_off)
		return 0;

	ret = smmu_add_cmd(smmu, cmd);
	if (ret)
		return ret;

	return smmu_sync_cmd(smmu);
}

static int smmu_sync_ste(struct hyp_arm_smmu_v3_device *smmu, u32 sid)
{
	struct arm_smmu_cmdq_ent cmd = {
		.opcode = CMDQ_OP_CFGI_STE,
		.cfgi.sid = sid,
		.cfgi.leaf = false,	/* Leaf=0 also invalidates cached CDs (IHI0070G §4.7) */
	};

	return smmu_send_cmd(smmu, &cmd);
}

static int smmu_sync_ste_range(struct hyp_arm_smmu_v3_device *smmu, u32 sid, u8 range)
{
	struct arm_smmu_cmdq_ent cmd = {
		.opcode = CMDQ_OP_CFGI_STE_RANGE,
		.cfgi.sid = sid,
		.cfgi.range = range,
	};

	return smmu_send_cmd(smmu, &cmd);
}

static int smmu_sync_cd(struct hyp_arm_smmu_v3_device *smmu, u32 sid, u32 ssid)
{
	struct arm_smmu_cmdq_ent cmd = {
		.opcode = CMDQ_OP_CFGI_CD,
		.cfgi.sid	= sid,
		.cfgi.ssid	= ssid,
		.cfgi.leaf = true,
	};

	return smmu_send_cmd(smmu, &cmd);
}

static int smmu_alloc_l2_strtab(struct hyp_arm_smmu_v3_device *smmu, u32 sid)
{
	struct arm_smmu_strtab_cfg *cfg = &smmu->strtab_cfg;
	struct arm_smmu_strtab_l1 *l1_desc;
	dma_addr_t l2ptr_dma;
	struct arm_smmu_strtab_l2 *l2table;
	size_t l2_order = get_order(sizeof(struct arm_smmu_strtab_l2));
	u32 l1_idx = arm_smmu_strtab_l1_idx(sid);

	if (l1_idx >= cfg->l2.num_l1_ents)
		return -EINVAL;

	l1_desc = &cfg->l2.l1tab[l1_idx];
	if (l1_desc->l2ptr)
		return 0;

	l2table = kvm_iommu_donate_pages(l2_order, 0);
	if (!l2table)
		return -ENOMEM;

	l2ptr_dma = hyp_virt_to_phys(l2table);

	/* Ensure the empty stream table is visible before the descriptor write */
	wmb();

	arm_smmu_write_strtab_l1_desc(l1_desc, l2ptr_dma);
	return 0;
}

static struct arm_smmu_ste *
smmu_get_ste_ptr(struct hyp_arm_smmu_v3_device *smmu, u32 sid)
{
	struct arm_smmu_strtab_cfg *cfg = &smmu->strtab_cfg;

	if (smmu->features & ARM_SMMU_FEAT_2_LVL_STRTAB) {
		struct arm_smmu_strtab_l1 *l1_desc;
		struct arm_smmu_strtab_l2 *l2ptr;
		u32 l1_idx = arm_smmu_strtab_l1_idx(sid);

		if (l1_idx >= cfg->l2.num_l1_ents)
			return NULL;
		l1_idx = array_index_nospec(l1_idx, cfg->l2.num_l1_ents);
		l1_desc = &cfg->l2.l1tab[l1_idx];
		/* L2 should be allocated before calling this. */
		if (WARN_ON(!l1_desc->l2ptr))
			return NULL;

		l2ptr = hyp_phys_to_virt(l1_desc->l2ptr & STRTAB_L1_DESC_L2PTR_MASK);
		/* Two-level walk */
		return &l2ptr->stes[arm_smmu_strtab_l2_idx(sid)];
	}

	if (sid >= cfg->linear.num_ents)
		return NULL;
	/* Simple linear lookup */
	sid = array_index_nospec(sid, cfg->linear.num_ents);
	return &cfg->linear.table[sid];
}

static int smmu_nested_cfg_sync(struct kvm_hyp_iommu *iommu, void *cmd_desc, size_t cmd_desc_size)
{
	struct hyp_arm_smmu_v3_cmd *cmd = cmd_desc;
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	struct arm_smmu_ste *ste;
	u32 sid, ssid;
	int ret;

	if (cmd_desc_size != sizeof(*cmd))
		return -EINVAL;
	else if (FIELD_GET(CMDQ_0_OP, le64_to_cpu(cmd->data[0])) != CMDQ_OP_CFGI_CD)
		return -EINVAL;

	/* Take the lock to avoid the STE from going away. */
	kvm_iommu_lock(iommu);
	sid = FIELD_GET(CMDQ_CFGI_0_SID, le64_to_cpu(cmd->data[0]));
	ste = smmu_get_ste_ptr(smmu, sid);
	if (!ste || (FIELD_GET(STRTAB_STE_0_CFG, le64_to_cpu(ste->data[0])) !=
				STRTAB_STE_0_CFG_NESTED)) {
		ret = -EINVAL;
		goto out;
	}

	ssid = FIELD_GET(CMDQ_CFGI_0_SSID, le64_to_cpu(cmd->data[0]));
	ret = smmu_sync_cd(smmu, sid, ssid);
	WARN_ON(ret);

out:
	kvm_iommu_unlock(iommu);
	return ret;
}

static struct arm_smmu_ste *
smmu_get_alloc_ste_ptr(struct hyp_arm_smmu_v3_device *smmu, u32 sid)
{
	if (smmu->features & ARM_SMMU_FEAT_2_LVL_STRTAB) {
		int ret = smmu_alloc_l2_strtab(smmu, sid);

		if (ret) {
			WARN_ON(ret != -ENOMEM);
			return NULL;
		}
	}
	return smmu_get_ste_ptr(smmu, sid);
}

static u64 *smmu_get_cd_ptr(u64 *cdtab, u32 ssid)
{
	/* Only linear supported for now. */
	return cdtab + ssid * CTXDESC_CD_DWORDS;
}

static phys_addr_t smmu_alloc_cd(struct hyp_arm_smmu_v3_device *smmu, u32 pasid_bits)
{
	u64 *cd_table;
	u32 requested_order = get_order((1 << pasid_bits) *
					(CTXDESC_CD_DWORDS << 3));

	/*
	 * We support max of 64K linear tables only, this should be enough
	 * for 128 pasids
	 */
	if (WARN_ON(requested_order > 4))
		return 0;

	cd_table = kvm_iommu_donate_pages(requested_order, 0);
	if (!cd_table)
		return 0;
	return hyp_virt_to_phys(cd_table);
}

static void smmu_free_cd(u64 *cd_table, u32 pasid_bits)
{
	u32 order = get_order((1 << pasid_bits) *
			      (CTXDESC_CD_DWORDS << 3));

	kvm_iommu_reclaim_pages(cd_table, order);
}

static int smmu_init_registers(struct hyp_arm_smmu_v3_device *smmu)
{
	u64 val, old;
	int ret;

	if (!(readl_relaxed(smmu->base + ARM_SMMU_GBPA) & GBPA_ABORT))
		return -EINVAL;

	/* Initialize all RW registers that will be read by the SMMU */
	ret = smmu_write_cr0(smmu, 0);
	if (ret)
		return ret;

	val = FIELD_PREP(CR1_TABLE_SH, ARM_SMMU_SH_ISH) |
	      FIELD_PREP(CR1_TABLE_OC, CR1_CACHE_WB) |
	      FIELD_PREP(CR1_TABLE_IC, CR1_CACHE_WB) |
	      FIELD_PREP(CR1_QUEUE_SH, ARM_SMMU_SH_ISH) |
	      FIELD_PREP(CR1_QUEUE_OC, CR1_CACHE_WB) |
	      FIELD_PREP(CR1_QUEUE_IC, CR1_CACHE_WB);
	writel_relaxed(val, smmu->base + ARM_SMMU_CR1);
	writel_relaxed(CR2_PTM, smmu->base + ARM_SMMU_CR2);

	val = readl_relaxed(smmu->base + ARM_SMMU_GERROR);
	old = readl_relaxed(smmu->base + ARM_SMMU_GERRORN);
	/* Service Failure Mode is fatal */
	if ((val ^ old) & GERROR_SFM_ERR)
		return -EIO;
	/* Clear pending errors */
	writel_relaxed(val, smmu->base + ARM_SMMU_GERRORN);

	return 0;
}

static void smmu_share_pages(phys_addr_t phys, size_t size)
{
	size_t nr_pages;
	u64 pfn;

	WARN_ON(!PAGE_ALIGNED(phys) || !PAGE_ALIGNED(size));
	nr_pages = size >> PAGE_SHIFT;
	pfn = phys >> PAGE_SHIFT;

	while (nr_pages--) {
		WARN_ON(__pkvm_host_share_hyp(pfn++));
	}
}

static int smmu_init_cmdq(struct hyp_arm_smmu_v3_device *smmu)
{
	u64 cmdq_base;
	size_t cmdq_nr_entries, cmdq_size;
	int ret;

	cmdq_base = readq_relaxed(smmu->base + ARM_SMMU_CMDQ_BASE);
	if (cmdq_base & ~(Q_BASE_RWA | Q_BASE_ADDR_MASK | Q_BASE_LOG2SIZE))
		return -EINVAL;

	smmu->cmdq_log2size = cmdq_base & Q_BASE_LOG2SIZE;
	cmdq_nr_entries = 1 << smmu->cmdq_log2size;
	cmdq_size = cmdq_nr_entries * CMDQ_ENT_DWORDS * 8;

	cmdq_base &= Q_BASE_ADDR_MASK;

	ret = ___pkvm_host_donate_hyp_prot(cmdq_base >> PAGE_SHIFT,
					   PAGE_ALIGN(cmdq_size) >> PAGE_SHIFT,
					   false, PAGE_HYP);
	if (ret)
		return ret;

	smmu->cmdq_base = hyp_phys_to_virt(cmdq_base);

	memset(smmu->cmdq_base, 0, cmdq_size);
	writel_relaxed(0, smmu->base + ARM_SMMU_CMDQ_PROD);
	writel_relaxed(0, smmu->base + ARM_SMMU_CMDQ_CONS);

	return 0;
}

/*
 * Event q support is optional and managed by the kernel,
 * However, it must set in a shared state so it can't be donated
 * to the hypervisor later.
 * This relies on the ARM_SMMU_EVTQ_BASE can't be changed after
 * de-privilege.
 */
static int smmu_init_evtq(struct hyp_arm_smmu_v3_device *smmu)
{
	u64 evtq_base, evtq_pfn;
	size_t evtq_nr_entries, evtq_size, evtq_nr_pages;
	void *evtq_va, *evtq_end;
	size_t i;
	int ret;

	evtq_base = readq_relaxed(smmu->base + ARM_SMMU_EVTQ_BASE);
	if (!evtq_base)
		return 0;

	if (evtq_base & ~(Q_BASE_RWA | Q_BASE_ADDR_MASK | Q_BASE_LOG2SIZE))
		return -EINVAL;

	evtq_nr_entries = 1 << (evtq_base & Q_BASE_LOG2SIZE);
	evtq_size = evtq_nr_entries * EVTQ_ENT_DWORDS * 8;
	evtq_nr_pages = PAGE_ALIGN(evtq_size) >> PAGE_SHIFT;

	evtq_pfn = PAGE_ALIGN(evtq_base & Q_BASE_ADDR_MASK) >> PAGE_SHIFT;

	for (i = 0 ; i < evtq_nr_pages ; ++i) {
		ret = __pkvm_host_share_hyp(evtq_pfn + i);
		if (ret)
			return ret;
	}

	evtq_va = hyp_phys_to_virt(evtq_pfn << PAGE_SHIFT);
	evtq_end = hyp_phys_to_virt((evtq_pfn + evtq_nr_pages) << PAGE_SHIFT);

	return hyp_pin_shared_mem(evtq_va, evtq_end);
}

static int smmu_init_strtab(struct hyp_arm_smmu_v3_device *smmu)
{
	int ret;
	u64 strtab_base;
	size_t strtab_size;
	u32 strtab_cfg, fmt;
	int split, log2size;
	struct arm_smmu_strtab_cfg *cfg = &smmu->strtab_cfg;

	strtab_base = readq_relaxed(smmu->base + ARM_SMMU_STRTAB_BASE);
	if (strtab_base & ~(STRTAB_BASE_ADDR_MASK | STRTAB_BASE_RA))
		return -EINVAL;

	strtab_cfg = readl_relaxed(smmu->base + ARM_SMMU_STRTAB_BASE_CFG);
	if (strtab_cfg & ~(STRTAB_BASE_CFG_FMT | STRTAB_BASE_CFG_SPLIT |
			   STRTAB_BASE_CFG_LOG2SIZE))
		return -EINVAL;

	fmt = FIELD_GET(STRTAB_BASE_CFG_FMT, strtab_cfg);
	split = FIELD_GET(STRTAB_BASE_CFG_SPLIT, strtab_cfg);
	log2size = FIELD_GET(STRTAB_BASE_CFG_LOG2SIZE, strtab_cfg);
	strtab_base &= STRTAB_BASE_ADDR_MASK;

	switch (fmt) {
	case STRTAB_BASE_CFG_FMT_LINEAR:
		if (split)
			return -EINVAL;
		cfg->linear.num_ents = 1 << log2size;
		strtab_size = cfg->linear.num_ents * sizeof(struct arm_smmu_ste);
		cfg->linear.ste_dma = strtab_base;
		ret = ___pkvm_host_donate_hyp_prot(strtab_base >> PAGE_SHIFT,
						   PAGE_ALIGN(strtab_size) >> PAGE_SHIFT,
						   false, PAGE_HYP);
		if (ret)
			return -EINVAL;
		cfg->linear.table = hyp_phys_to_virt(strtab_base);
		/* Disable all STEs */
		memset(cfg->linear.table, 0, strtab_size);
		break;
	case STRTAB_BASE_CFG_FMT_2LVL:
		if (split != STRTAB_SPLIT)
			return -EINVAL;
		cfg->l2.num_l1_ents = 1 << max(0, log2size - split);
		strtab_size = cfg->l2.num_l1_ents * sizeof(struct arm_smmu_strtab_l1);
		cfg->l2.l1_dma = strtab_base;
		ret = ___pkvm_host_donate_hyp_prot(strtab_base >> PAGE_SHIFT,
						   PAGE_ALIGN(strtab_size) >> PAGE_SHIFT,
						   false, PAGE_HYP);
		if (ret)
			return -EINVAL;
		cfg->l2.l1tab = hyp_phys_to_virt(strtab_base);
		/* Disable all STEs */
		memset(cfg->l2.l1tab, 0, strtab_size);
		break;
	default:
		return -EINVAL;
	}
	return 0;
}

static int smmu_reset_device(struct hyp_arm_smmu_v3_device *smmu)
{
	int ret;
	struct arm_smmu_cmdq_ent cfgi_cmd = {
		.opcode = CMDQ_OP_CFGI_STE_RANGE,
		.cfgi.range = 31,
	};
	struct arm_smmu_cmdq_ent tlbi_cmd = {
		.opcode = CMDQ_OP_TLBI_NSNH_ALL,
	};

	/* Invalidate all cached configs and TLBs */
	ret = smmu_write_cr0(smmu, CR0_CMDQEN);
	if (ret)
		return ret;

	ret = smmu_add_cmd(smmu, &cfgi_cmd);
	if (ret)
		goto err_disable_cmdq;

	ret = smmu_add_cmd(smmu, &tlbi_cmd);
	if (ret)
		goto err_disable_cmdq;

	ret = smmu_sync_cmd(smmu);
	if (ret)
		goto err_disable_cmdq;

	/* Enable translation */
	return smmu_write_cr0(smmu, CR0_SMMUEN | CR0_CMDQEN | CR0_ATSCHK | CR0_EVTQEN);

err_disable_cmdq:
	return smmu_write_cr0(smmu, 0);
}

static void smmu_smc_set_power_state(struct hyp_arm_smmu_v3_device *smmu, bool on)
{
	struct arm_smccc_res res;
	u32 state = on ? 1 : 0;

	arm_smccc_1_1_smc(SMC_FC_OEM_NS_SMMU_STATE, smmu->mmio_addr >> FIRMWARE_PAGE_SHIFT,
			  state, &res);
	WARN_ON(res.a0 != SMCCC_RET_SUCCESS);
}

static bool smmu_needs_fw_sync(struct hyp_arm_smmu_v3_device *smmu)
{
	return (smmu->options & ARM_SMMU_OPT_SYNC_FW) && kvm_hyp_smmu_global_config.use_smc_s2;
}

static int smmu_init_device(struct hyp_arm_smmu_v3_device *smmu)
{
	int ret;

	if (!PAGE_ALIGNED(smmu->mmio_addr | smmu->mmio_size))
		return -EINVAL;

	ret = ___pkvm_host_donate_hyp(smmu->mmio_addr >> PAGE_SHIFT,
				      smmu->mmio_size >> PAGE_SHIFT,
				      /* accept_mmio */ true);
	if (ret)
		return ret;

	smmu->base = hyp_phys_to_virt(smmu->mmio_addr);

	/* Driver doesn't support non-coherent SMMUs. */
	if (!(smmu->features & ARM_SMMU_FEAT_COHERENCY))
		return -EIO;

	ret = smmu_init_registers(smmu);
	if (ret)
		return ret;

	ret = smmu_init_cmdq(smmu);
	if (ret)
		return ret;

	ret = smmu_init_evtq(smmu);
	if (ret)
		return ret;

	ret = smmu_init_strtab(smmu);
	if (ret)
		return ret;

	ret = smmu_reset_device(smmu);
	if (ret)
		return ret;

	/* Let firmware know the SMMU has been initialized. */
	if (smmu_needs_fw_sync(smmu))
		smmu_smc_set_power_state(smmu, true);

	return kvm_iommu_init_device(&smmu->iommu);
}

static int smmu_init(void)
{
	int ret;
	struct hyp_arm_smmu_v3_device *smmu;
	size_t smmu_arr_size = PAGE_ALIGN(sizeof(*kvm_hyp_arm_smmu_v3_smmus) *
					  kvm_hyp_arm_smmu_v3_count);
	phys_addr_t smmu_arr_phys;

	kvm_hyp_arm_smmu_v3_smmus = kern_hyp_va(kvm_hyp_arm_smmu_v3_smmus);

	smmu_arr_phys = hyp_virt_to_phys(kvm_hyp_arm_smmu_v3_smmus);

	ret = smmu_take_pages(smmu_arr_phys, smmu_arr_size);
	if (ret)
		return ret;

	kvm_hyp_smmu_last_err = kern_hyp_va(kvm_hyp_smmu_last_err);
	smmu_share_pages(hyp_virt_to_phys(kvm_hyp_smmu_last_err),
			 PAGE_ALIGN(NR_CPUS * sizeof(struct hyp_arm_smmu_v3_err)));

	kvm_hyp_shared_arm_smmu_telemetry = kern_hyp_va(kvm_hyp_shared_arm_smmu_telemetry);
	smmu_share_pages(hyp_virt_to_phys(kvm_hyp_shared_arm_smmu_telemetry),
			 PAGE_ALIGN(sizeof(*kvm_hyp_shared_arm_smmu_telemetry)));

	/* Guaranteed to be set by KVM */
	arch_timer_rate = read_sysreg(cntfrq_el0);
	smmu_poll_timeout_cycle = ((u64)ARM_SMMU_POLL_TIMEOUT_US * arch_timer_rate / 1000000);
	smmu_init_telemetry(arch_timer_rate);

	for_each_smmu(smmu) {
		ret = smmu_init_device(smmu);
		if (ret)
			goto out_reclaim_smmu;
	}

	return 0;
out_reclaim_smmu:
	smmu_reclaim_pages(smmu_arr_phys, smmu_arr_size);
	return ret;
}

static struct kvm_hyp_iommu *smmu_id_to_iommu(pkvm_handle_t smmu_id)
{
	if (smmu_id >= kvm_hyp_arm_smmu_v3_count)
		return NULL;
	smmu_id = array_index_nospec(smmu_id, kvm_hyp_arm_smmu_v3_count);

	return &kvm_hyp_arm_smmu_v3_smmus[smmu_id].iommu;
}

static struct hyp_arm_smmu_v3_domain *smmu_alloc_nested_domain(void)
{
	struct hyp_arm_smmu_v3_nested_domain *nested_domain = hyp_alloc(sizeof(*nested_domain));

	if (!nested_domain)
		return NULL;

	return &nested_domain->smmu_domain;
}

static int smmu_alloc_domain(struct kvm_hyp_iommu_domain *domain, int type)
{
	struct hyp_arm_smmu_v3_domain *smmu_domain;

	if (type >= KVM_ARM_SMMU_DOMAIN_MAX)
		return -EINVAL;

	if (type == KVM_ARM_SMMU_DOMAIN_NESTED)
		smmu_domain = smmu_alloc_nested_domain();
	else
		smmu_domain = hyp_alloc(sizeof(*smmu_domain));

	if (!smmu_domain) {
		kvm_iommu_request_hyp_alloc();
		return -ENOMEM;
	}

	INIT_LIST_HEAD(&smmu_domain->iommu_list);
	hyp_rwlock_init(&smmu_domain->list_lock);
	/*
	 * Can't do much without knowing the SMMUv3.
	 * Page table will be allocated at attach_dev, but can be
	 * freed from free domain.
	 */
	smmu_domain->domain = domain;
	smmu_domain->type = type;
	smmu_domain->snapshot_done = false;
	hyp_spin_lock_init(&smmu_domain->pgt_lock);
	domain->priv = (void *)smmu_domain;
	if (domain->domain_id < MAX_SMMU_DOMAIN) {
		smmu_telemetry_init_domain_data(domain->domain_id);

		hyp_spin_lock(&smmu_domains_lock);
		smmu_domains[domain->domain_id] = smmu_domain;
		hyp_spin_unlock(&smmu_domains_lock);
	}

	return 0;
}

static bool smmu_is_domain_nested(struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	return smmu_domain->type == KVM_ARM_SMMU_DOMAIN_NESTED;
}

static void smmu_free_domain(struct kvm_hyp_iommu_domain *domain)
{
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct domain_iommu_node *iommu_node, *temp;

	if (smmu_domain->pgtable)
		kvm_arm_io_pgtable_free_pixel(smmu_domain->pgtable);

	/*
	 * With device assignment it is possible to free a domain with attached devices,
	 * they will be disabled through dev_block_dma op.
	 * In that case free the IOMMU nodes to avoid leaking memory.
	 */
	list_for_each_entry_safe(iommu_node, temp, &smmu_domain->iommu_list, list) {
		list_del(&iommu_node->list);
		hyp_free(iommu_node);
	}

	if (smmu_domain->cdptr) {
		/*
		 * pasid_bits = 0 is the expected size parameter
		 * for the shared CD table based on how smmu_alloc_cd
		 * uses that argument in smmu_domain_config_s1.
		 */
		smmu_free_cd(hyp_phys_to_virt(smmu_domain->cdptr), 0);
	}

	if (domain->domain_id < MAX_SMMU_DOMAIN) {
		hyp_spin_lock(&smmu_domains_lock);
		smmu_domains[domain->domain_id] = NULL;
		hyp_spin_unlock(&smmu_domains_lock);
	}

	if (smmu_is_domain_nested(smmu_domain))
		hyp_free(to_nested_domain(smmu_domain));
	else
		hyp_free(smmu_domain);
}

/* Must be protected by smmu.iommu.lock */
static void smmu_record_timeout(struct hyp_arm_smmu_v3_device *smmu)
{
	int cpu = hyp_smp_processor_id();
	struct hyp_arm_smmu_v3_err *err = &kvm_hyp_smmu_last_err[cpu];

	err->smmu_id = smmu - kvm_hyp_arm_smmu_v3_smmus;
	err->type = HYP_ARM_SMMU_V3_ERR_CMDQ_TIMEOUT;
	err->cmdq_cons = readl_relaxed(smmu->base + ARM_SMMU_CMDQ_CONS);
	err->cmdq_prod = readl_relaxed(smmu->base + ARM_SMMU_CMDQ_PROD);
	err->cmdq_prod_sw = smmu->cmdq_prod;
}

static void smmu_inv_domain(struct hyp_arm_smmu_v3_device *smmu,
			    struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	struct kvm_hyp_iommu_domain *domain = smmu_domain->domain;
	struct arm_smmu_cmdq_ent cmd = {};

	cmd.tlbi.vmid = smmu_domain->vmid;
	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_S2) {
		cmd.opcode = CMDQ_OP_TLBI_S12_VMALL;
	} else if (smmu_is_domain_nested(smmu_domain)) {
		/* Guest controls CD.ASID; hyp domain_id is unrelated. */
		cmd.opcode = CMDQ_OP_TLBI_NH_ALL;
	} else {
		cmd.opcode = CMDQ_OP_TLBI_NH_ASID;
		cmd.tlbi.asid = domain->domain_id;
	}

	if (smmu->iommu.power_is_off)
		return;

	WARN_ON(smmu_send_cmd(smmu, &cmd));
}

static void smmu_tlb_flush_all(void *cookie)
{
	struct kvm_hyp_iommu_domain *domain = cookie;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct hyp_arm_smmu_v3_device *smmu;
	struct domain_iommu_node *iommu_node;

	hyp_read_lock(&smmu_domain->list_lock);
	list_for_each_entry(iommu_node, &smmu_domain->iommu_list, list) {
		smmu = to_smmu(iommu_node->iommu);
		kvm_iommu_lock(&smmu->iommu);
		smmu_inv_domain(smmu, smmu_domain);
		kvm_iommu_unlock(&smmu->iommu);
	}
	hyp_read_unlock(&smmu_domain->list_lock);
}

static void smmu_cmdq_batch_add(struct hyp_arm_smmu_v3_device *smmu,
				struct arm_smmu_cmdq_batch *cmds,
				struct arm_smmu_cmdq_ent *cmd)
{
	int index;

	if (cmds->num == CMDQ_BATCH_ENTRIES) {
		WARN_ON(smmu_issue_cmds(smmu, cmds->cmds, cmds->num));
		cmds->num = 0;
	}

	index = cmds->num * CMDQ_ENT_DWORDS;
	smmu_build_cmd(&cmds->cmds[index], cmd);

	cmds->num++;
}

static void smmu_tlb_inv_vmalle1(struct hyp_arm_smmu_v3_device *smmu,
				 struct arm_smmu_cmdq_batch *cmds, u16 vmid)
{
	struct arm_smmu_cmdq_ent cmd = {
		.opcode = CMDQ_OP_TLBI_NH_ALL,
		.tlbi.vmid = vmid,
	};

	smmu_cmdq_batch_add(smmu, cmds, &cmd);
}

static int smmu_tlb_inv_range_smmu(struct hyp_arm_smmu_v3_device *smmu,
				   struct kvm_hyp_iommu_domain *domain,
				   struct arm_smmu_cmdq_ent *cmd,
				   unsigned long iova, size_t size, size_t granule)
{
	int ret = 0;
	unsigned long end = iova + size, num_pages = 0, tg = 0;
	size_t inv_range = granule;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct arm_smmu_cmdq_batch cmds = { };

	kvm_iommu_lock(&smmu->iommu);
	if (smmu->iommu.power_is_off)
		goto out_ret;

	/* Almost copy-paste from the kernel dirver. */
	if (smmu->features & ARM_SMMU_FEAT_RANGE_INV) {
		/* Get the leaf page size */
		tg = __ffs(smmu_domain->pgsize_bitmap);

		num_pages = size >> tg;

		/* Convert page size of 12,14,16 (log2) to 1,2,3 */
		cmd->tlbi.tg = (tg - 10) / 2;

		/*
		 * Determine what level the granule is at. For non-leaf, both
		 * io-pgtable and SVA pass a nominal last-level granule because
		 * they don't know what level(s) actually apply, so ignore that
		 * and leave TTL=0. However for various errata reasons we still
		 * want to use a range command, so avoid the SVA corner case
		 * where both scale and num could be 0 as well.
		 */
		if (cmd->tlbi.leaf)
			cmd->tlbi.ttl = 4 - ((ilog2(granule) - 3) / (tg - 3));
		else if ((num_pages & CMDQ_TLBI_RANGE_NUM_MAX) == 1)
			num_pages++;
	}

	cmds.num = 0;

	while (iova < end) {
		if (smmu->features & ARM_SMMU_FEAT_RANGE_INV) {
			/*
			 * On each iteration of the loop, the range is 5 bits
			 * worth of the aligned size remaining.
			 * The range in pages is:
			 *
			 * range = (num_pages & (0x1f << __ffs(num_pages)))
			 */
			unsigned long scale, num;

			/* Determine the power of 2 multiple number of pages */
			scale = __ffs(num_pages);
			cmd->tlbi.scale = scale;

			/* Determine how many chunks of 2^scale size we have */
			num = (num_pages >> scale) & CMDQ_TLBI_RANGE_NUM_MAX;
			cmd->tlbi.num = num - 1;

			/* range is num * 2^scale * pgsize */
			inv_range = num << (scale + tg);

			/* Clear out the lower order bits for the next iteration */
			num_pages -= num << scale;
		}
		cmd->tlbi.addr = iova;
		smmu_cmdq_batch_add(smmu, &cmds, cmd);
		BUG_ON(iova + inv_range < iova);
		iova += inv_range;
	}

	if (smmu_domain->num_nested_s1)
		smmu_tlb_inv_vmalle1(smmu, &cmds, cmd->tlbi.vmid);

	WARN_ON(smmu_issue_cmds(smmu, cmds.cmds, cmds.num));
	ret = smmu_sync_cmd(smmu);
	if (ret == -ETIMEDOUT)
		smmu_record_timeout(smmu);
out_ret:
	kvm_iommu_unlock(&smmu->iommu);
	return ret;
}

static void smmu_smc_inv_ns_ipa_start(unsigned long iova, size_t size)
{
	struct arm_smccc_res res;

	arm_smccc_1_1_smc(SMC_FC_OEM_INV_NS_IPA_START, iova >> FIRMWARE_PAGE_SHIFT,
			  size >> FIRMWARE_PAGE_SHIFT, &res);
	WARN_ON(res.a0 != SMCCC_RET_SUCCESS);
}

static void smmu_smc_inv_ns_ipa_complete(void)
{
	struct arm_smccc_res res;

	arm_smccc_1_1_smc(SMC_FC_OEM_INV_NS_IPA_COMPLETE, &res);
	WARN_ON(res.a0 != SMCCC_RET_SUCCESS);
}

static void smmu_tlb_inv_range(struct kvm_hyp_iommu_domain *domain,
			       unsigned long iova, size_t size, size_t granule,
			       bool leaf)
{
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct hyp_arm_smmu_v3_device *smmu;
	struct domain_iommu_node *iommu_node;
	unsigned long end = iova + size;
	struct arm_smmu_cmdq_ent cmd = { };
	bool sync_tlb_inv = kvm_hyp_smmu_global_config.use_smc_s2 &&
			    domain->domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID;

	cmd.tlbi.leaf = leaf;
	cmd.tlbi.vmid = smmu_domain->vmid;
	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_S2) {
		cmd.opcode = CMDQ_OP_TLBI_S2_IPA;
		smmu_telemetry_inc_s2_tlb_invals();
	} else {
		cmd.opcode = CMDQ_OP_TLBI_NH_VA;
		cmd.tlbi.asid = domain->domain_id;
	}
	/*
	 * There are no mappings at high addresses since we don't use TTB1, so
	 * no overflow possible.
	 */
	BUG_ON(end < iova);

	if (sync_tlb_inv)
		smmu_smc_inv_ns_ipa_start(iova, size);

	hyp_read_lock(&smmu_domain->list_lock);
	list_for_each_entry(iommu_node, &smmu_domain->iommu_list, list) {
		smmu = to_smmu(iommu_node->iommu);
		WARN_ON(smmu_tlb_inv_range_smmu(smmu, domain, &cmd, iova, size, granule));
	}
	hyp_read_unlock(&smmu_domain->list_lock);

	if (sync_tlb_inv)
		smmu_smc_inv_ns_ipa_complete();
}

static void smmu_unmap_visit_leaf(phys_addr_t addr, size_t size,
				  struct io_pgtable_walk_common *data,
				  void *wd)
{
	u64 *ptep = wd;
	u64 pte = *ptep;

	/* Might be a cleared table. */
	if (!pte)
		return;

	/* Skip still-valid contiguous neighbours */
	if (pte & ARM_LPAE_PTE_VALID)
		return;

	WARN_ON(iommu_pkvm_unuse_dma(addr, size));
	*ptep = 0;
}

static void smmu_iotlb_inv_nested_domain(struct kvm_hyp_iommu_domain *domain, unsigned long iova,
					 size_t size, size_t granule, bool leaf)
{
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;

	if (!smmu_is_domain_nested(smmu_domain))
		return;

	if (iova == 0 && size == ULONG_MAX)
		smmu_tlb_flush_all(domain);
	else
		smmu_tlb_inv_range(domain, iova, size, granule, leaf);
}

static void smmu_tlb_flush_walk(unsigned long iova, size_t size,
				size_t granule, void *cookie)
{
	struct kvm_hyp_iommu_domain *domain = cookie;

	smmu_tlb_inv_range(domain, iova, size, granule, false);
}

static void smmu_tlb_add_page(struct iommu_iotlb_gather *gather,
			      unsigned long iova, size_t granule,
			      void *cookie)
{
	if (gather)
		kvm_iommu_iotlb_gather_add_page(cookie, gather, iova, granule);
	else
		smmu_tlb_inv_range(cookie, iova, granule, granule, true);
}

static void smmu_free_leaf(unsigned long phys, size_t granule, void *cookie)
{
	struct kvm_hyp_iommu_domain *domain = cookie;

	/* No tracking for idmap domain. */
	if (domain->domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID)
		return;

	WARN_ON(iommu_pkvm_unuse_dma(phys, granule));
}

static const struct iommu_flush_ops smmu_tlb_ops = {
	.tlb_flush_all	= smmu_tlb_flush_all,
	.tlb_flush_walk = smmu_tlb_flush_walk,
	.tlb_add_page	= smmu_tlb_add_page,
	.free_leaf	= smmu_free_leaf,
};

static void smmu_iotlb_sync(struct kvm_hyp_iommu_domain *domain,
			    struct iommu_iotlb_gather *gather)
{
	size_t size;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct io_pgtable *pgtable = smmu_domain->pgtable;
	struct arm_lpae_io_pgtable *data = io_pgtable_to_data(pgtable);
	struct arm_lpae_io_pgtable_walk_data wd = {
		.cookie = data,
	};
	struct io_pgtable_walk_common walk_data = {
		.visit_leaf = smmu_unmap_visit_leaf,
		.data = &wd,
	};

	if (!gather->pgsize)
		return;
	size = gather->end - gather->start + 1;
	smmu_tlb_inv_range(domain, gather->start, size,  gather->pgsize, true);

	/*
	 * Now decrement the refcount of unmapped pages thanks to
	 * IO_PGTABLE_QUIRK_UNMAP_INVAL
	 */
	pgtable->ops.pgtable_walk(&pgtable->ops, gather->start, size, &walk_data);
}

static int smmu_domain_config_s2(struct kvm_hyp_iommu_domain *domain,
				 struct arm_smmu_ste *ste,
				 bool force_cacheable)
{
	struct io_pgtable_cfg *cfg;
	u64 ts, sl, ic, oc, sh, tg, ps;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;

	cfg = &smmu_domain->pgtable->cfg;
	ps = cfg->arm_lpae_s2_cfg.vtcr.ps;
	tg = cfg->arm_lpae_s2_cfg.vtcr.tg;
	sh = cfg->arm_lpae_s2_cfg.vtcr.sh;
	oc = cfg->arm_lpae_s2_cfg.vtcr.orgn;
	ic = cfg->arm_lpae_s2_cfg.vtcr.irgn;
	sl = cfg->arm_lpae_s2_cfg.vtcr.sl;
	ts = cfg->arm_lpae_s2_cfg.vtcr.tsz;

	ste->data[0] = STRTAB_STE_0_V |
		FIELD_PREP(STRTAB_STE_0_CFG, STRTAB_STE_0_CFG_S2_TRANS);
	if (force_cacheable)
		ste->data[1] = FIELD_PREP(STRTAB_STE_1_SHCFG, STRTAB_STE_1_SHCFG_ISH) |
				FIELD_PREP(STRTAB_STE_1_MEMATTR, ARM_SMMU_MEMATTR_OIWB) |
				STRTAB_STE_1_MTCFG;
	else
		ste->data[1] = FIELD_PREP(STRTAB_STE_1_SHCFG, STRTAB_STE_1_SHCFG_INCOMING);
	ste->data[2] = FIELD_PREP(STRTAB_STE_2_VTCR,
			FIELD_PREP(STRTAB_STE_2_VTCR_S2PS, ps) |
			FIELD_PREP(STRTAB_STE_2_VTCR_S2TG, tg) |
			FIELD_PREP(STRTAB_STE_2_VTCR_S2SH0, sh) |
			FIELD_PREP(STRTAB_STE_2_VTCR_S2OR0, oc) |
			FIELD_PREP(STRTAB_STE_2_VTCR_S2IR0, ic) |
			FIELD_PREP(STRTAB_STE_2_VTCR_S2SL0, sl) |
			FIELD_PREP(STRTAB_STE_2_VTCR_S2T0SZ, ts)) |
		 FIELD_PREP(STRTAB_STE_2_S2VMID, smmu_domain->vmid) |
		 STRTAB_STE_2_S2AA64 | STRTAB_STE_2_S2R;
	ste->data[3] = cfg->arm_lpae_s2_cfg.vttbr & STRTAB_STE_3_S2TTB_MASK;

	return 0;
}

static void smmu_domain_config_s1_ste(phys_addr_t cd_table,
				      u32 pasid_bits, u16 vmid, struct arm_smmu_ste *ste)
{
	ste->data[2] = FIELD_PREP(STRTAB_STE_2_S2VMID, vmid);
	ste->data[1] = FIELD_PREP(STRTAB_STE_1_S1DSS, STRTAB_STE_1_S1DSS_SSID0) |
		FIELD_PREP(STRTAB_STE_1_S1CIR, STRTAB_STE_1_S1C_CACHE_WBRA) |
		FIELD_PREP(STRTAB_STE_1_S1COR, STRTAB_STE_1_S1C_CACHE_WBRA) |
		FIELD_PREP(STRTAB_STE_1_S1CSH, ARM_SMMU_SH_ISH);
	ste->data[0] = (cd_table & STRTAB_STE_0_S1CTXPTR_MASK) |
		FIELD_PREP(STRTAB_STE_0_CFG, STRTAB_STE_0_CFG_S1_TRANS) |
		FIELD_PREP(STRTAB_STE_0_S1CDMAX, pasid_bits) |
		FIELD_PREP(STRTAB_STE_0_S1FMT, STRTAB_STE_0_S1FMT_LINEAR) |
		STRTAB_STE_0_V;
}

/*
 * This function handles configuration for pasid and non-pasid domains
 * with the following assumptions:
 * - pasid 0 always attached first, this should be the typicall flow
 *   for the kernel where attach_dev is always called before set_dev_pasid.
 *   In that case only pasid 0 is allowed to allocate memory for the CD,
 *   and other pasids would expect to find the tabel.
 * - pasid 0 is detached last, also guaranteed from the kernel.
 */
static int smmu_domain_config_s1(struct hyp_arm_smmu_v3_device *smmu,
				 struct kvm_hyp_iommu_domain *domain,
				 u32 sid, u32 pasid, u32 pasid_bits,
				 struct arm_smmu_ste *ste)
{
	struct arm_smmu_ste *dst;
	u64 val;
	u64 *cd_entry;
	phys_addr_t cd_table;
	struct io_pgtable_cfg *cfg;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;

	cfg = &smmu_domain->pgtable->cfg;
	dst = smmu_get_ste_ptr(smmu, sid);
	val = dst->data[0];

	/*
	 * Only an empty STE or one we previously installed as pure S1 is a
	 * legal target for S1 (re)configuration. In particular, reject NESTED
	 * STEs — their S1CTXPTR was authored by the (untrusted) host and is
	 * an IPA, not a hyp-owned PA.
	 */
	if (val && FIELD_GET(STRTAB_STE_0_CFG, val) != STRTAB_STE_0_CFG_S1_TRANS)
		return -EBUSY;

	/*
	 * CD ptr is retrieved differently for stage-1:
	 * - PASID != 0 attach, it's read from the STE as PASID = 0 Must be attached first.
	 * - PASID = 0 and PASID_BITs > 0: Devices that support pasids, allocate a new CD table.
	 * - PASID_BITS = 0: Share the CDptr inside the domain if existing, otherwise allocate it.
	 */
	if (!pasid_bits) {
		/* Single-entry CD table — only pasid 0 is addressable. */
		if (pasid)
			return -EINVAL;
		/* The domain already has a CD table. */
		if (smmu_domain->cdptr) {
			cd_table = smmu_domain->cdptr;
		} else {
			cd_table = smmu_alloc_cd(smmu, 0);
			if (!cd_table)
				return -ENOMEM;
			smmu_domain->cdptr = cd_table;
		}
	} else {
		if (pasid == 0) {
			cd_table = smmu_alloc_cd(smmu, pasid_bits);
			if (!cd_table)
				return -ENOMEM;
		} else {
			u32 nr_entries;

			/*
			 * pasid!=0 attach is only valid if *this* hyp instance
			 * installed the pasid==0 STE first; otherwise S1CTXPTR
			 * cannot be trusted as a hyp-owned PA.
			 */
			if (FIELD_GET(STRTAB_STE_0_CFG, val) != STRTAB_STE_0_CFG_S1_TRANS)
				return -EINVAL;
			cd_table = (FIELD_GET(STRTAB_STE_0_S1CTXPTR_MASK, val) << 6);
			if (!cd_table)
				return -EINVAL;
			nr_entries = 1 << FIELD_GET(STRTAB_STE_0_S1CDMAX, val);
			if (pasid >= nr_entries)
				return -E2BIG;
		}
	}

	/* PASID = 0 always populates the STE. */
	if (pasid == 0)
		smmu_domain_config_s1_ste(cd_table, pasid_bits, smmu_domain->vmid, ste);

	/* Write CD. */
	cd_entry = smmu_get_cd_ptr(hyp_phys_to_virt(cd_table), pasid);

	if (cd_entry[0]) {
		/* CD already attached to a different domain. */
		if (cfg->arm_lpae_s1_cfg.ttbr != (cd_entry[1] & CTXDESC_CD_1_TTB0_MASK))
			return -EBUSY;
		/*
		 * This can only happen for PASID = 0 && PASID_BITS = 0 with shared cd table,
		 * in that case the STE is not live, so no need to sync the CD or write it.
		 */
		return 0;
	}

	cd_entry[1] = cpu_to_le64(cfg->arm_lpae_s1_cfg.ttbr & CTXDESC_CD_1_TTB0_MASK);
	cd_entry[2] = 0;
	cd_entry[3] = cpu_to_le64(cfg->arm_lpae_s1_cfg.mair);

	/* STE is live. */
	if (pasid)
		WARN_ON(smmu_sync_cd(smmu, sid, pasid));
	val =  FIELD_PREP(CTXDESC_CD_0_TCR_T0SZ, cfg->arm_lpae_s1_cfg.tcr.tsz) |
	       FIELD_PREP(CTXDESC_CD_0_TCR_TG0, cfg->arm_lpae_s1_cfg.tcr.tg) |
	       FIELD_PREP(CTXDESC_CD_0_TCR_IRGN0, cfg->arm_lpae_s1_cfg.tcr.irgn) |
	       FIELD_PREP(CTXDESC_CD_0_TCR_ORGN0, cfg->arm_lpae_s1_cfg.tcr.orgn) |
	       FIELD_PREP(CTXDESC_CD_0_TCR_SH0, cfg->arm_lpae_s1_cfg.tcr.sh) |
	       FIELD_PREP(CTXDESC_CD_0_TCR_IPS, cfg->arm_lpae_s1_cfg.tcr.ips) |
	       CTXDESC_CD_0_TCR_EPD1 | CTXDESC_CD_0_AA64 |
	       CTXDESC_CD_0_R | CTXDESC_CD_0_A |
	       CTXDESC_CD_0_ASET |
	       FIELD_PREP(CTXDESC_CD_0_ASID, domain->domain_id) |
	       CTXDESC_CD_0_V;
	WRITE_ONCE(cd_entry[0], cpu_to_le64(val));
	/* STE is live. */
	if (pasid)
		WARN_ON(smmu_sync_cd(smmu, sid, pasid));
	return 0;
}

static int smmu_domain_finalise(struct hyp_arm_smmu_v3_device *smmu,
				struct kvm_hyp_iommu_domain *domain,
				unsigned long flags)
{
	int ret;
	struct io_pgtable_cfg cfg;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct arm_lpae_io_pgtable *data;
	bool idmapped = domain->domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID;
	unsigned long quirks = idmapped ? 0 : IO_PGTABLE_QUIRK_UNMAP_INVAL;


	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_S1) {
		size_t ias = (smmu->features & ARM_SMMU_FEAT_VAX) ? 52 : 48;
		bool coherent_walk = !(smmu->options & ARM_SMMU_OPT_NON_COHERENT_TTW);

		if (flags & ARM_SMMU_DISABLE_CONTPTE)
			quirks |= IO_PGTABLE_QUIRK_DISABLE_CONTPTE;

		cfg = (struct io_pgtable_cfg) {
			.fmt = ARM_64_LPAE_S1,
			.pgsize_bitmap = smmu->pgsize_bitmap,
			.ias = min_t(unsigned long, ias, VA_BITS),
			.oas = smmu->ias,
			.coherent_walk = coherent_walk,
			.tlb = &smmu_tlb_ops,
			.quirks = quirks,
			.telemetry_cb = &smmu_telemetry_cb,
		};
	} else {
		bool coherent_walk = kvm_hyp_smmu_global_config.s2_non_coherent_ttw ?
			false : smmu->features & ARM_SMMU_FEAT_COHERENCY;

		cfg = (struct io_pgtable_cfg) {
			.fmt = ARM_64_LPAE_S2,
			.pgsize_bitmap = smmu->pgsize_bitmap,
			.ias = smmu->ias,
			.oas = smmu->oas,
			.coherent_walk = coherent_walk,
			.tlb = &smmu_tlb_ops,
			.quirks = quirks,
			.telemetry_cb = &smmu_telemetry_cb,
		};
	}

	hyp_spin_lock(&smmu_domain->pgt_lock);
	smmu_domain->pgtable = kvm_arm_io_pgtable_alloc_pixel(&cfg, domain, &ret);
	smmu_domain->pgsize_bitmap = cfg.pgsize_bitmap;
	hyp_spin_unlock(&smmu_domain->pgt_lock);
	if (ret)
		return ret;

	data = io_pgtable_to_data(smmu_domain->pgtable);
	data->idmapped = idmapped;
	smmu_domain->pgtable->cfg.defer_sync_pte = !data->idmapped;
	return ret;
}

static bool smmu_domain_compat(struct hyp_arm_smmu_v3_device *smmu,
			       struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	struct io_pgtable_cfg *cfg;

	/* Domain is empty. */
	if (!smmu_domain->pgtable)
		return true;

	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_S2) {
		if (!(smmu->features & ARM_SMMU_FEAT_TRANS_S2))
			return false;
	} else {
		if (!(smmu->features & ARM_SMMU_FEAT_TRANS_S1))
			return false;
	}

	cfg = &smmu_domain->pgtable->cfg;

	/* Best effort. */
	return  ((smmu->pgsize_bitmap | cfg->pgsize_bitmap) == smmu->pgsize_bitmap);
}

static int smmu_get_ref_domain(struct hyp_arm_smmu_v3_device *smmu,
			       struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	struct domain_iommu_node *iommu_node;
	struct hyp_arm_smmu_v3_device *other;

	hyp_assert_write_lock_held(&smmu_domain->list_lock);

	list_for_each_entry(iommu_node, &smmu_domain->iommu_list, list) {
		other = to_smmu(iommu_node->iommu);
		if (other == smmu) {
			iommu_node->ref++;
			return 0;
		}
	}

	if (!smmu_domain_compat(smmu, smmu_domain))
		return -EBUSY;

	iommu_node = hyp_alloc(sizeof(*iommu_node));
	if (!iommu_node) {
		kvm_iommu_request_hyp_alloc();
		return -ENOMEM;
	}

	iommu_node->iommu = &smmu->iommu;
	iommu_node->ref = 1;
	list_add_tail(&iommu_node->list, &smmu_domain->iommu_list);

	return 0;
}

static void smmu_put_ref_domain(struct hyp_arm_smmu_v3_device *smmu,
				struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	struct domain_iommu_node *iommu_node, *temp;
	struct hyp_arm_smmu_v3_device *other;

	hyp_assert_write_lock_held(&smmu_domain->list_lock);

	list_for_each_entry_safe(iommu_node, temp, &smmu_domain->iommu_list, list) {
		other = to_smmu(iommu_node->iommu);
		if (other == smmu) {
			iommu_node->ref--;
			if (iommu_node->ref == 0) {
				/*
				 * Ensure no stale tlb entries when domain_id
				 * is re-used for this SMMU.
				 */
				smmu_inv_domain(smmu, smmu_domain);

				list_del(&iommu_node->list);
				hyp_free(iommu_node);
			}
			return;
		}
	}
}

static int smmu_fix_up_domains(struct hyp_arm_smmu_v3_device *smmu,
			       struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	/*
	 * BYPASS domains only supported on stage-2 instances, that is over restrictive
	 * but for now as stage-1 is limited to VA_BITS to match the kernel, it might
	 * not cover the ia bits, we don't support it.
	 */
	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_BYPASS) {
		if (smmu->features & ARM_SMMU_FEAT_TRANS_S2)
			smmu_domain->type = KVM_ARM_SMMU_DOMAIN_S2;
		else
			return -EINVAL;
	} else if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_ANY) {
		/* Any domain defaults to S1 as we don't know if the guest needs pasid. */
		if (smmu->features & ARM_SMMU_FEAT_TRANS_S1) {
			smmu_domain->type = KVM_ARM_SMMU_DOMAIN_S1;
		} else {
			smmu_domain->type = KVM_ARM_SMMU_DOMAIN_S2;
		}
	}

	return 0;
}

static struct arm_smmu_ste *smmu_get_valid_ste_ptr(struct hyp_arm_smmu_v3_device *smmu, u32 sid)
{
	struct arm_smmu_ste *ste;

	ste = smmu_get_ste_ptr(smmu, sid);
	if (!ste)
		return NULL;

	if (!(ste->data[0] & STRTAB_STE_0_V))
		return NULL;

	return ste;
}

static int smmu_ste_cont_apply(struct hyp_arm_smmu_v3_device *smmu, u32 sid, u32 ste_cont, u32 val)
{
	int i;
	struct arm_smmu_ste *ste;
	int sids_num = (1UL << ste_cont);

	for (i = 0; i < sids_num; i++) {
		ste = smmu_get_ste_ptr(smmu, sid++);
		if (!ste)
			return -EINVAL;

		ste->data[1] &= ~STRTAB_STE_1_CONT;
		ste->data[1] |= FIELD_PREP(STRTAB_STE_1_CONT, val);
	}

	return 0;
}

static int smmu_ste_cont_aggregate(struct hyp_arm_smmu_v3_device *smmu, u32 last_sid)
{
	int l1_idx, l2_idx, i, ret;
	u32 base_sid, sid, cnt, max_cnt, ste_cont;
	struct arm_smmu_strtab_cfg *cfg = &smmu->strtab_cfg;
	struct arm_smmu_strtab_l1 *l1_desc;
	struct arm_smmu_ste *last_ste;
	struct arm_smmu_ste *ste;
	bool match, aggregate;

	if (!(smmu->features & ARM_SMMU_FEAT_2_LVL_STRTAB))
		return 0;

	last_ste = smmu_get_valid_ste_ptr(smmu, last_sid);
	if (!last_ste)
		return -EINVAL;

	aggregate = false;

	for (l1_idx = 0; l1_idx < cfg->l2.num_l1_ents; l1_idx++) {
		l1_desc = &cfg->l2.l1tab[l1_idx];
		if (!l1_desc->l2ptr)
			continue;

		l2_idx = 0;
		while (l2_idx < STRTAB_NUM_L2_STES) {
			sid = l1_idx * STRTAB_NUM_L2_STES + l2_idx;
			match = false;
			ste = smmu_get_valid_ste_ptr(smmu, sid);
			if (ste) {
				match = true;
				for (i = 0; i < STRTAB_STE_DWORDS; i++) {
					if (ste->data[i] != last_ste->data[i]) {
						match = false;
						break;
					}
				}
			}

			/*
			 * Step 1: Keep skipping SIDs until there is one divisible
			 * by a power of 2.
			 */
			if (!aggregate) {
				if (!match) {
					l2_idx++;
					continue;
				}

				/* Do not allow past the end of L2 */
				max_cnt = STRTAB_NUM_L2_STES - l2_idx;
				if (sid)
					max_cnt = min(max_cnt, (1UL << __ffs(sid)));

				if (max_cnt == 1) {
					l2_idx++;
					continue;
				}

				base_sid = sid;
				cnt = 1;
				aggregate = true;
				l2_idx++;
				continue;
			}

			/*
			 * Step 2: Look ahead to find the length
			 * of the current consecutive SID block.
			 */
			if (match) {
				cnt++;
				if (cnt < max_cnt) {
					l2_idx++;
					continue;
				}
			}

			if (cnt == 1) {
				aggregate = false;
				l2_idx++;
				continue;
			}

			ste_cont = __fls(cnt);
			ret = smmu_ste_cont_apply(smmu, base_sid, ste_cont, ste_cont);
			if (ret)
				return ret;

			ret = smmu_sync_ste_range(smmu, base_sid, ste_cont - 1);
			WARN_ON(ret);
			if (ret)
				return ret;

			l2_idx = arm_smmu_strtab_l2_idx(base_sid) + (1UL << ste_cont);
			aggregate = false;
		}
	}

	return 0;
}

static void smmu_set_ns_ipa_tbl(struct hyp_arm_smmu_v3_domain *smmu_domain)
{
	struct arm_lpae_io_pgtable *data = io_pgtable_to_data(smmu_domain->pgtable);
	u64 tbl_base = hyp_virt_to_phys(data->pgd);
	u32 sl0 = smmu_domain->pgtable->cfg.arm_lpae_s2_cfg.vtcr.sl;
	u32 tg = smmu_domain->pgtable->cfg.arm_lpae_s2_cfg.vtcr.tg;
	u32 t0sz = smmu_domain->pgtable->cfg.arm_lpae_s2_cfg.vtcr.tsz;
	bool coherent_walk = smmu_domain->pgtable->cfg.coherent_walk;
	u32 ipa_cfg;
	struct arm_smccc_res res;

	ipa_cfg = (coherent_walk ? SET_NS_IPA_CFG_COHERENT_WALK : 0) |
		  FIELD_PREP(SET_NS_IPA_CFG_GRANULE, tg) |
		  FIELD_PREP(SET_NS_IPA_CFG_START_LVL, sl0) |
		  FIELD_PREP(SET_NS_IPA_CFG_T0SZ, t0sz);

	arm_smccc_1_1_smc(SMC_FC_OEM_SET_NS_IPA_TBL, tbl_base >> FIRMWARE_PAGE_SHIFT, ipa_cfg,
			  &res);
	WARN_ON(res.a0 != SMCCC_RET_SUCCESS);
}

static int smmu_write_ste(struct hyp_arm_smmu_v3_device *smmu, u32 sid,
			  const struct arm_smmu_ste *ste)
{
	struct arm_smmu_ste *dst = smmu_get_ste_ptr(smmu, sid);
	int i, ret;

	/*
	 * The SMMU may cache a disabled STE.
	 * Initialize all fields, sync, then enable it.
	 */
	for (i = 1; i < STRTAB_STE_DWORDS; i++)
		dst->data[i] = ste->data[i];

	ret = smmu_sync_ste(smmu, sid);
	if (ret)
		return ret;

	WRITE_ONCE(dst->data[0], ste->data[0]);
	return smmu_sync_ste(smmu, sid);
}

static int smmu_domain_config_nested(struct hyp_arm_smmu_v3_device *smmu, u32 sid,
				     struct kvm_hyp_iommu_domain *s2_domain,
				     const struct hyp_arm_smmu_v3_s1_ctx *s1_ste,
				     unsigned long flags,
				     struct arm_smmu_ste *ste)
{
	int ret;

	/* Validate incoming STE. */
	if (!(s1_ste->ste[0] & STRTAB_STE_0_V) ||
	    (FIELD_GET(STRTAB_STE_0_CFG, s1_ste->ste[0]) == STRTAB_STE_0_CFG_ABORT))
		return -EINVAL;

	ret = smmu_domain_config_s2(s2_domain, ste, flags & ARM_SMMU_FORCE_CACHEABLE);
	if (ret)
		return ret;

	ste->data[0] = STRTAB_STE_0_V | FIELD_PREP(STRTAB_STE_0_CFG, STRTAB_STE_0_CFG_NESTED);
	ste->data[0] |= s1_ste->ste[0] & ~STRTAB_STE_0_CFG;
	/*
	 * Never let the host enable EATS=TRANS (ATC-cached PA bypasses S2 even
	 * with ATSCHK=1; hyp issues no CMD_ATC_INV) or STRW=EL2 (would be
	 * ILLEGAL with Config[1]=1 anyway, but make it structural).
	 */
	ste->data[1] |= s1_ste->ste[1] &
			~(STRTAB_STE_1_CONT | STRTAB_STE_1_EATS | STRTAB_STE_1_STRW);

	if (smmu->options & ARM_SMMU_OPT_OVR_INSTCFG_DATA)
		ste->data[1] |= FIELD_PREP(STRTAB_STE_1_INSTCFG, STRTAB_STE_1_INSTCFG_DATA);

	if (flags & ARM_SMMU_TCU_PREFETCH_FORWARD)
		ste->data[1] |= FIELD_PREP(STRTAB_STE_1_TCU_PREFETCH,
					   STRTAB_STE_1_TCU_PREFETCH_FORWARD);
	else if (flags & ARM_SMMU_TCU_PREFETCH_BACKWARD)
		ste->data[1] |= FIELD_PREP(STRTAB_STE_1_TCU_PREFETCH,
					   STRTAB_STE_1_TCU_PREFETCH_BACKWARD);

	return 0;
}

static int smmu_install_nested_ste(struct hyp_arm_smmu_v3_device *smmu, u32 sid,
				   struct kvm_hyp_iommu_domain *s2_domain,
				   struct hyp_arm_smmu_v3_s1_ctx *s1_ste, unsigned long flags)
{
	struct arm_smmu_ste ste = {};
	struct arm_smmu_ste *dst = smmu_get_alloc_ste_ptr(smmu, sid);
	int ret;

	if (!dst)
		return -ENOMEM;
	else if (dst->data[0])
		return -EBUSY;

	ret = smmu_domain_config_nested(smmu, sid, s2_domain, s1_ste, flags, &ste);
	if (ret)
		return ret;

	ret = smmu_write_ste(smmu, sid, &ste);
	WARN_ON(ret);
	if (ret)
		return ret;

	if (flags & ARM_SMMU_ATTACH_DEV_LAST_SID)
		WARN_ON(smmu_ste_cont_aggregate(smmu, sid));

	return ret;
}

static int smmu_attach_dev_nested(struct kvm_hyp_iommu *iommu,
				  struct kvm_hyp_iommu_domain *s1_domain,
				  struct kvm_hyp_iommu_domain *s2_domain, u32 sid, u32 pasid,
				  unsigned long flags, void *s1_desc, size_t s1_desc_size)
{
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	struct hyp_arm_smmu_v3_domain *s1_smmu_domain = s1_domain->priv;
	struct hyp_arm_smmu_v3_domain *s2_smmu_domain = s2_domain->priv;
	struct hyp_arm_smmu_v3_s1_ctx *s1_ste = s1_desc;
	bool init_idmap = false;
	int ret;

	hyp_write_lock(&s1_smmu_domain->list_lock);
	hyp_write_lock(&s2_smmu_domain->list_lock);
	kvm_iommu_lock(iommu);

	/* Setup the S2 idmap domain first. */
	ret = smmu_fix_up_domains(smmu, s2_smmu_domain);
	if (ret)
		goto out_unlock;

	if (pasid != 0) {
		ret = -EINVAL;
		goto out_unlock;
	}

	/*
	 * We allow KVM_ARM_SMMU_DOMAIN_BYPASS since that's the only stage-2 domain we support at
	 * the moment for nested domains.
	 */
	if (!(smmu->features & ARM_SMMU_FEAT_NESTING) ||
	    !smmu_is_domain_nested(s1_smmu_domain) ||
	    s2_smmu_domain->type != KVM_ARM_SMMU_DOMAIN_S2 ||
	    s2_domain->domain_id != KVM_IOMMU_DOMAIN_IDMAP_ID ||
	    s1_desc_size != sizeof(*s1_ste)) {
		ret = -EINVAL;
		goto out_unlock;
	}
	s2_smmu_domain->vmid = s2_domain->domain_id;

	ret = smmu_get_ref_domain(smmu, s2_smmu_domain);
	if (ret)
		goto out_unlock;

	if (!s2_smmu_domain->pgtable) {
		ret = smmu_domain_finalise(smmu, s2_domain, 0);
		if (ret)
			goto out_unlock_s2_ref;
	}

	if (!s2_smmu_domain->snapshot_done)
		init_idmap = true;

	/* Then handle the S1 domain. */
	s1_smmu_domain->vmid = s2_smmu_domain->vmid;
	/*
	 * Note: This assumes that the host and hypervisor have the same page sizes. This
	 * must be updated if this assumption is no longer true.
	 */
	s1_smmu_domain->pgsize_bitmap = PAGE_SIZE;

	ret = smmu_get_ref_domain(smmu, s1_smmu_domain);
	if (ret)
		goto out_unlock_s2_ref;

	if (pasid == 0) {
		ret = smmu_install_nested_ste(smmu, sid, s2_domain, s1_ste, flags);
		if (!ret)
			s2_smmu_domain->num_nested_s1++;
	}

	if (ret)
		smmu_put_ref_domain(smmu, s1_smmu_domain);
out_unlock_s2_ref:
	if (ret)
		smmu_put_ref_domain(smmu, s2_smmu_domain);
out_unlock:
	kvm_iommu_unlock(iommu);
	hyp_write_unlock(&s2_smmu_domain->list_lock);
	hyp_write_unlock(&s1_smmu_domain->list_lock);

	if (!ret && init_idmap) {
		/*
		 * Setup the table first on the firmware side because the snapshot can trigger
		 * TLB invalidations if pagetable entries are being coalesced. If that happens
		 * before the table is setup, the firmware will be invoked to perform TLB
		 * invalidations without knowledge of the table.
		 */
		if (kvm_hyp_smmu_global_config.use_smc_s2)
			smmu_set_ns_ipa_tbl(s2_smmu_domain);

		kvm_hyp_shared_arm_smmu_telemetry->hs2t.host_mem_usage = 0;
		ret = kvm_iommu_snapshot_host_stage2(s2_domain);
		if (!ret) {
			s2_smmu_domain->snapshot_done = true;
			smmu_telemetry_rec_idmap_snapshot();
		}
	}

	return ret;
}

static int smmu_attach_dev(struct kvm_hyp_iommu *iommu, struct kvm_hyp_iommu_domain *domain,
			   u32 sid, u32 pasid, u32 pasid_bits, unsigned long flags)
{
	int ret;
	struct arm_smmu_ste *dst;
	struct arm_smmu_ste ste = {};
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	bool force_cacheable;
	bool init_idmap = false;

	hyp_write_lock(&smmu_domain->list_lock);
	kvm_iommu_lock(iommu);

	if (smmu_is_domain_nested(smmu_domain)) {
		ret = -EINVAL;
		goto out_unlock;
	}

	dst = smmu_get_alloc_ste_ptr(smmu, sid);
	if (!dst) {
		ret = -ENOMEM;
		goto out_unlock;
	}

	force_cacheable = flags & ARM_SMMU_FORCE_CACHEABLE;

	/* Map domain type to an SMMUv3 stage. */
	ret = smmu_fix_up_domains(smmu, smmu_domain);
	if (ret)
		goto out_unlock;

	ret = smmu_get_ref_domain(smmu, smmu_domain);
	if (ret)
		goto out_unlock;

	if (!smmu_domain->pgtable) {
		ret = smmu_domain_finalise(smmu, domain, flags);
		if (ret)
			goto out_unlock_ref;
	}

	if (domain->domain_id == KVM_IOMMU_DOMAIN_IDMAP_ID &&
	    !smmu_domain->snapshot_done)
		init_idmap = true;

	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_S2) {
		/* Device already attached or pasid for s2. */
		if (dst->data[0] || pasid) {
			ret = -EBUSY;
			goto out_unlock_ref;
		}
		smmu_domain->vmid = domain->domain_id;
		ret = smmu_domain_config_s2(domain, &ste, force_cacheable);
	} else {
		/*
		 * Allocate and config CD, and update CD if possible.
		 */
		pasid_bits = min(pasid_bits, smmu->ssid_bits);
		smmu_domain->vmid = KVM_SMMU_S1_DOMAIN_VMID;
		ret = smmu_domain_config_s1(smmu, domain, sid, pasid,
					    pasid_bits, &ste);
	}
	/* We don't update STEs for pasid domains. */
	if (ret || pasid)
		goto out_unlock_ref;

	if (smmu->options & ARM_SMMU_OPT_OVR_INSTCFG_DATA) {
		ste.data[1] |= FIELD_PREP(STRTAB_STE_1_INSTCFG,
					  STRTAB_STE_1_INSTCFG_DATA);
	}

	if (flags & ARM_SMMU_TCU_PREFETCH_FORWARD) {
		ste.data[1] |= FIELD_PREP(STRTAB_STE_1_TCU_PREFETCH,
					  STRTAB_STE_1_TCU_PREFETCH_FORWARD);
	} else if (flags & ARM_SMMU_TCU_PREFETCH_BACKWARD) {
		ste.data[1] |= FIELD_PREP(STRTAB_STE_1_TCU_PREFETCH,
					  STRTAB_STE_1_TCU_PREFETCH_BACKWARD);
	}

	ret = smmu_write_ste(smmu, sid, &ste);
	WARN_ON(ret);
	if (ret)
		goto out_unlock_ref;

	if (flags & ARM_SMMU_ATTACH_DEV_LAST_SID)
		WARN_ON(smmu_ste_cont_aggregate(smmu, sid));

out_unlock_ref:
	if (ret)
		smmu_put_ref_domain(smmu, smmu_domain);
out_unlock:
	kvm_iommu_unlock(iommu);
	hyp_write_unlock(&smmu_domain->list_lock);

	if (!ret && init_idmap) {
		/*
		 * Setup the table first on the firmware side because the snapshot can trigger
		 * TLB invalidations if pagetable entries are being coalesced. If that happens
		 * before the table is setup, the firmware will be invoked to perform TLB
		 * invalidations without knowledge of the table.
		 */
		if (kvm_hyp_smmu_global_config.use_smc_s2)
			smmu_set_ns_ipa_tbl(smmu_domain);

		kvm_hyp_shared_arm_smmu_telemetry->hs2t.host_mem_usage = 0;
		ret = kvm_iommu_snapshot_host_stage2(domain);
		if (!ret) {
			smmu_domain->snapshot_done = true;
			smmu_telemetry_rec_idmap_snapshot();
		}
	}

	return ret;
}

static int smmu_clear_ste(struct hyp_arm_smmu_v3_device *smmu, u32 sid)
{
	struct arm_smmu_ste *dst = smmu_get_ste_ptr(smmu, sid);
	int i, ret;

	dst->data[0] = 0;
	ret = smmu_sync_ste(smmu, sid);
	WARN_ON(ret);
	if (ret)
		return ret;
	for (i = 1; i < STRTAB_STE_DWORDS; i++)
		dst->data[i] = 0;

	return WARN_ON(smmu_sync_ste(smmu, sid));
}

static int smmu_detach_dev(struct kvm_hyp_iommu *iommu, struct kvm_hyp_iommu_domain *domain,
			   u32 sid, u32 pasid)
{
	struct arm_smmu_ste *dst;
	int ret;
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	u32 pasid_bits = 0;
	phys_addr_t cd_table = 0;
	u64 *cd;
	u32 domain_id, ste_cfg, ste_cont, base_sid;

	hyp_write_lock(&smmu_domain->list_lock);
	kvm_iommu_lock(iommu);
	if (smmu_is_domain_nested(smmu_domain)) {
		ret = -EINVAL;
		goto out_unlock;
	}

	dst = smmu_get_ste_ptr(smmu, sid);
	if (!dst) {
		ret = -ENODEV;
		goto out_unlock;
	}

	ste_cfg = FIELD_GET(STRTAB_STE_0_CFG, dst->data[0]);
	/*
	 * For stage-1:
	 * - The kernel has to detach pasid = 0 the last.
	 * - This will free the CD.
	 *
	 * Look at smmu_domain_config_s1 for CD allocation and life time
	 * For detach stage-1 domains:
	 * - PASID != 0, invalidate that the CD entry, as leave the table as it might be used.
	 * - PASID = 0 and PASID_BITS > 0: Devices that support pasids, free the CD table and
	 *   invalidate the STE.
	 * - PASID_BITS = 0: invalidate the STE if STE.CONT = 0. If STE is part of the range,
	 *   first remove all STE's from the range. The cdptr per domain is freed at free_domain().
	 */
	if (smmu_domain->type == KVM_ARM_SMMU_DOMAIN_S1) {
		if (ste_cfg != STRTAB_STE_0_CFG_S1_TRANS) {
			ret = -EACCES;
			goto out_unlock;
		}
		pasid_bits = FIELD_GET(STRTAB_STE_0_S1CDMAX, dst->data[0]);
		if (pasid >= (1 << pasid_bits)) {
			ret = -E2BIG;
			goto out_unlock;
		}
		cd_table = (dst->data[0] & STRTAB_STE_0_S1CTXPTR_MASK);
		if (WARN_ON(!cd_table)) {
			ret = -ENODEV;
			goto out_unlock;
		}

		if (pasid == 0) {
			int j;

			/* Ensure other pasids are detached. */
			for (j = 1 ; j < (1 << pasid_bits) ; ++j) {
				cd = smmu_get_cd_ptr(hyp_phys_to_virt(cd_table), j);
				if (cd[0] & CTXDESC_CD_0_V) {
					ret = -EINVAL;
					goto out_unlock;
				}
			}
			cd = smmu_get_cd_ptr(hyp_phys_to_virt(cd_table), 0);
			domain_id = FIELD_GET(CTXDESC_CD_0_ASID, cd[0]);
			if (domain->domain_id != domain_id) {
				ret = -EACCES;
				goto out_unlock;
			}
		} else {
			cd = smmu_get_cd_ptr(hyp_phys_to_virt(cd_table), pasid);
			if (!(cd[0] & CTXDESC_CD_0_V)) {
				/* The device is not actually attached! */
				ret = -ENOENT;
				goto out_unlock;
			}
			domain_id = FIELD_GET(CTXDESC_CD_0_ASID, cd[0]);
			if (domain->domain_id != domain_id) {
				ret = -EACCES;
				goto out_unlock;
			}
			cd[0] = 0;
			ret = smmu_sync_cd(smmu, sid, pasid);
			WARN_ON(ret);
			if (ret)
				goto out_unlock;
			cd[1] = 0;
			cd[2] = 0;
			cd[3] = 0;
			ret = smmu_sync_cd(smmu, sid, pasid);
			WARN_ON(ret);
			if (ret)
				goto out_unlock;
			smmu_put_ref_domain(smmu, smmu_domain);
			goto out_unlock;
		}
	} else {
		domain_id = FIELD_GET(STRTAB_STE_2_S2VMID, dst->data[2]);
		if ((ste_cfg != STRTAB_STE_0_CFG_S2_TRANS) ||
		    (domain->domain_id != domain_id)) {
			ret = -EACCES;
			goto out_unlock;
		}
	}
	/* For stage-2 and pasid = 0 */
	ste_cont = FIELD_GET(STRTAB_STE_1_CONT, dst->data[1]);
	if (ste_cont) {
		/* Find start of the range */
		base_sid = (sid & ~((1UL << ste_cont) - 1));

		ret = smmu_ste_cont_apply(smmu, base_sid, ste_cont, 0);
		WARN_ON(ret);

		ret = smmu_sync_ste_range(smmu, base_sid, ste_cont - 1);
		WARN_ON(ret);
	}
	ret = smmu_clear_ste(smmu, sid);
	if (ret)
		goto out_unlock;

	smmu_put_ref_domain(smmu, smmu_domain);
out_unlock:
	kvm_iommu_unlock(iommu);
	hyp_write_unlock(&smmu_domain->list_lock);
	/* CD table is per SID and not domain for device with PASIDs. */
	if (!ret && cd_table && pasid == 0 && pasid_bits)
		smmu_free_cd(hyp_phys_to_virt(cd_table), pasid_bits);
	return ret;
}

static int smmu_detach_dev_nested(struct kvm_hyp_iommu *iommu,
				  struct kvm_hyp_iommu_domain *s1_domain,
				  struct kvm_hyp_iommu_domain *s2_domain, u32 sid, u32 pasid)
{
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	struct hyp_arm_smmu_v3_domain *s1_smmu_domain = s1_domain->priv;
	struct hyp_arm_smmu_v3_domain *s2_smmu_domain = s2_domain->priv;
	struct arm_smmu_ste *dst;
	u32 ste_cfg, base_sid, ste_cont;
	u16 vmid;
	int ret = 0;

	hyp_write_lock(&s1_smmu_domain->list_lock);
	hyp_write_lock(&s2_smmu_domain->list_lock);
	kvm_iommu_lock(iommu);

	if (!smmu_is_domain_nested(s1_smmu_domain)) {
		ret = -EINVAL;
		goto out;
	}

	dst = smmu_get_ste_ptr(smmu, sid);
	if (!dst) {
		ret = -ENODEV;
		goto out;
	}

	ste_cfg = FIELD_GET(STRTAB_STE_0_CFG, le64_to_cpu(dst->data[0]));
	vmid = FIELD_GET(STRTAB_STE_2_S2VMID, le64_to_cpu(dst->data[2]));
	if (ste_cfg != STRTAB_STE_0_CFG_NESTED || vmid != s2_domain->domain_id) {
		ret = -EACCES;
		goto out;
	}

	/*
	 * The STE is only installed by attach_dev_nested(pasid==0); a
	 * non-zero-pasid detach must not be allowed to drop the S1/S2
	 * iommu_node refs that pin this SMMU on the idmap's TLBI list
	 * while leaving the STE live.
	 */
	if (pasid != 0) {
		ret = -EINVAL;
		goto out;
	}

	if (pasid == 0) {
		ste_cont = FIELD_GET(STRTAB_STE_1_CONT, le64_to_cpu(dst->data[1]));
		if (ste_cont) {
			/* Find start of the range */
			base_sid = (sid & ~((1UL << ste_cont) - 1));

			ret = smmu_ste_cont_apply(smmu, base_sid, ste_cont, 0);
			WARN_ON(ret);

			ret = smmu_sync_ste_range(smmu, base_sid, ste_cont - 1);
			WARN_ON(ret);
		}

		ret = smmu_clear_ste(smmu, sid);
		if (ret)
			goto out;
		/*
		 * TLBI_S2_IPA is not architecturally required to invalidate
		 * cached combined S1+S2 entries (IHI0070G CMD_TLBI_S2_IPA).
		 * smmu_tlb_inv_range_smmu() compensates with TLBI_NH_ALL while
		 * num_nested_s1 > 0; once we drop to 0 that compensation stops,
		 * so flush all VMID-0 S1/combined entries now, before any
		 * subsequent idmap S2 change can rely on S2_IPA alone.
		 */
		if (--s2_smmu_domain->num_nested_s1 == 0) {
			struct arm_smmu_cmdq_ent cmd = {
				.opcode    = CMDQ_OP_TLBI_NH_ALL,
				.tlbi.vmid = s2_smmu_domain->vmid,
			};
			WARN_ON(smmu_send_cmd(smmu, &cmd));
		}
	}

	smmu_put_ref_domain(smmu, s1_smmu_domain);
	smmu_put_ref_domain(smmu, s2_smmu_domain);

out:
	kvm_iommu_unlock(iommu);
	hyp_write_unlock(&s2_smmu_domain->list_lock);
	hyp_write_unlock(&s1_smmu_domain->list_lock);
	return ret;
}

static int smmu_map_pages(struct kvm_hyp_iommu_domain *domain, unsigned long iova,
			  phys_addr_t paddr, size_t pgsize,
			  size_t pgcount, int prot, size_t *total_mapped)
{
	size_t mapped;
	size_t granule;
	int ret = 0;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct io_pgtable *pgtable = smmu_domain->pgtable;

	if (!pgtable)
		return -EINVAL;

	granule = 1UL << __ffs(smmu_domain->pgsize_bitmap);
	if (!IS_ALIGNED(iova | paddr | pgsize, granule))
		return -EINVAL;
	/*
	 * We cannot trust the host that iova and paddr are pgsize aligned.
	 */
	if (!IS_ALIGNED(iova | paddr, pgsize))
		return -EINVAL;

	hyp_spin_lock(&smmu_domain->pgt_lock);
	while (pgcount) {
		mapped = 0;
		ret = pgtable->ops.map_pages(&pgtable->ops, iova, paddr,
					     pgsize, pgcount, prot, 0, &mapped);
		if (ret)
			break;
		WARN_ON(!IS_ALIGNED(mapped, pgsize));
		WARN_ON(mapped > pgcount * pgsize);

		pgcount -= mapped / pgsize;
		*total_mapped += mapped;
		iova += mapped;
		paddr += mapped;
	}

	if (*total_mapped)
		smmu_domain->pgt_version++;

	hyp_spin_unlock(&smmu_domain->pgt_lock);

	return ret;
}

static size_t smmu_unmap_pages(struct kvm_hyp_iommu_domain *domain, unsigned long iova,
			       size_t pgsize, size_t pgcount, struct iommu_iotlb_gather *gather)
{
	size_t granule, unmapped, total_unmapped = 0;
	size_t size = pgsize * pgcount;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct io_pgtable *pgtable = smmu_domain->pgtable;

	if (!pgtable)
		return -EINVAL;

	granule = 1UL << __ffs(smmu_domain->pgsize_bitmap);
	if (!IS_ALIGNED(iova | pgsize, granule))
		return 0;
	/*
	 * We cannot trust the host that iova is pgsize aligned.
	 */
	if (!IS_ALIGNED(iova, pgsize))
		return 0;

	hyp_spin_lock(&smmu_domain->pgt_lock);
	while (total_unmapped < size) {
		unmapped = pgtable->ops.unmap_pages(&pgtable->ops, iova, pgsize,
						    pgcount, gather);
		if (!unmapped)
			break;
		iova += unmapped;
		total_unmapped += unmapped;
		pgcount -= unmapped / pgsize;
	}

	/*
	 * Eagerly drain the gather list before the core code does to Keep
	 * the page table walk locked.
	 */
	smmu_iotlb_sync(domain, gather);
	gather->pgsize = 0;
	iommu_iotlb_gather_init(gather);

	if (total_unmapped)
		smmu_domain->pgt_version++;

	hyp_spin_unlock(&smmu_domain->pgt_lock);
	return total_unmapped;
}

static phys_addr_t smmu_iova_to_phys(struct kvm_hyp_iommu_domain *domain,
				     unsigned long iova)
{
	phys_addr_t paddr;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct io_pgtable *pgtable = smmu_domain->pgtable;

	if (!pgtable)
		return -EINVAL;

	hyp_spin_lock(&smmu_domain->pgt_lock);
	paddr = pgtable->ops.iova_to_phys(&pgtable->ops, iova);
	hyp_spin_unlock(&smmu_domain->pgt_lock);

	return paddr;
}

static bool smmu_dabt_device(struct hyp_arm_smmu_v3_device *smmu,
			     struct user_pt_regs *regs,
			     u64 esr, u32 off)
{
	bool is_write = esr & ESR_ELx_WNR;
	unsigned int len = BIT((esr & ESR_ELx_SAS) >> ESR_ELx_SAS_SHIFT);
	int rd = (esr & ESR_ELx_SRT_MASK) >> ESR_ELx_SRT_SHIFT;
	const u32 no_access  = 0;
	const u32 read_write = (u32)(-1);
	const u32 read_only = is_write ? no_access : read_write;
	u32 mask = no_access;
	u32 allowed_len = 0;

	/*
	 * Only handle MMIO access with u32 or u64 size and alignment.
	 */
	switch (off) {
	case ARM_SMMU_EVTQ_PROD + SZ_64K:
		mask = read_write;
		allowed_len = BIT(sizeof(u32));
		break;
	case ARM_SMMU_EVTQ_CONS + SZ_64K:
		mask = read_write;
		allowed_len = BIT(sizeof(u32));
		break;
	case ARM_SMMU_GERROR:
		mask = read_only;
		allowed_len = BIT(sizeof(u32));
		break;
	case ARM_SMMU_GERRORN:
		mask = read_write;
		allowed_len = BIT(sizeof(u32));
		break;
	default:
		if (smmu->features & ARM_SMMU_FEAT_PMCG_PAGE0_2000 &&
		    off >= 0x2000 && off < 0x2000 + ARM_SMMU_PMCG_REG_SZ) {
			mask = read_write;
			allowed_len = BIT(sizeof(u32)) | BIT(sizeof(u64));
		}
		break;
	};

	if (!mask)
		return false;
	if (!(allowed_len & BIT(len)))
		return false;
	if (off & (len - 1))
		return false;

	if (len == sizeof(u32)) {
		if (is_write)
			writel_relaxed(regs->regs[rd] & mask, smmu->base + off);
		else
			regs->regs[rd] = readl_relaxed(smmu->base + off);
	} else if (len == sizeof(u64)) {
		if (is_write)
			writeq_relaxed(regs->regs[rd], smmu->base + off);
		else
			regs->regs[rd] = readq_relaxed(smmu->base + off);
	}

	return true;
}

static int smmu_id_to_token(pkvm_handle_t smmu_id, u64 *out_token)
{
	if (smmu_id >= kvm_hyp_arm_smmu_v3_count)
		return -EINVAL;

	smmu_id = array_index_nospec(smmu_id, kvm_hyp_arm_smmu_v3_count);
	*out_token = kvm_hyp_arm_smmu_v3_smmus[smmu_id].mmio_addr;
	return 0;
}

static int smmu_dev_block_dma(struct kvm_hyp_iommu *iommu, u32 sid, bool is_host2guest)
{
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	static struct arm_smmu_ste *dst;
	int ret = 0;

	kvm_iommu_lock(iommu);
	dst = smmu_get_ste_ptr(smmu, sid);

	/*
	 * VFIO will attach the device to a blocking domain, this will make the
	 * kernel driver detach the device which should be have zeroed STE.
	 * So, if this is not the current state of the device, something
	 * went wrong.
	 * For guests, Only clear STE, the CD will be freed when domains are
	 * freed on guest teardown.
	 */
	if (dst->data[0]) {
		if (is_host2guest) {
			ret = -EINVAL;
		} else {
			int i = 0;

			/* zap zippity zop. */
			for (i = 0; i < STRTAB_STE_DWORDS; i++)
				dst->data[i] = 0;

			ret = smmu_sync_ste(smmu, sid);
			WARN_ON(ret);
		}
	}

	kvm_iommu_unlock(iommu);
	return ret;
}

static bool smmu_dabt_handler(struct user_pt_regs *regs, u64 esr, u64 addr)
{
	struct hyp_arm_smmu_v3_device *smmu;

	for_each_smmu(smmu) {
		if (addr < smmu->mmio_addr || addr >= smmu->mmio_addr + smmu->mmio_size)
			continue;
		return smmu_dabt_device(smmu, regs, esr, addr - smmu->mmio_addr);
	}
	return false;
}

static int smmu_suspend(struct kvm_hyp_iommu *iommu)
{
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	int ret = 0;

	/*
	 * Disable translation, GBPA is validated at probe to be set, so all translation
	 * would be aborted when SMMU is disabled.
	 */
	if (iommu->power_domain.type == KVM_POWER_DOMAIN_HOST_HVC) {
		ret = smmu_write_cr0(smmu, 0);
		if (ret)
			return ret;

		if (smmu_needs_fw_sync(smmu))
			smmu_smc_set_power_state(smmu, false);
	}

	return ret;
}

static int smmu_resume(struct kvm_hyp_iommu *iommu)
{
	struct hyp_arm_smmu_v3_device *smmu = to_smmu(iommu);
	int ret = 0;

	/*
	 * Re-enable and clean all caches.
	 */
	if (iommu->power_domain.type == KVM_POWER_DOMAIN_HOST_HVC) {
		ret = smmu_reset_device(smmu);
		if (ret)
			return ret;

		if (smmu_needs_fw_sync(smmu))
			smmu_smc_set_power_state(smmu, true);
	}

	return ret;
}

static int smmu_iotlb_sync_map(struct kvm_hyp_iommu_domain *domain,
			       unsigned long iova, size_t size)
{
	int ret = 0;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;

	hyp_spin_lock(&smmu_domain->pgt_lock);
	if (smmu_domain->pgtable->ops.iotlb_sync_map)
		ret = smmu_domain->pgtable->ops.iotlb_sync_map(&smmu_domain->pgtable->ops,
							       iova, size);
	hyp_spin_unlock(&smmu_domain->pgt_lock);

	return ret;
}

/*
 * Although SMMU can support multiple granules, it must at least support PAGE_SIZE
 * as the CPU, and for the IDMAP domains, we only use this granule.
 * As we optimize for memory usage and performance, we try to use block mappings
 * when possible.
 */
static size_t smmu_pgsize_idmap(size_t size, u64 paddr)
{
	size_t pgsizes;
	size_t pgsize_bitmask = 0;

	if (PAGE_SIZE == SZ_4K) {
		pgsize_bitmask = SZ_4K | SZ_2M | SZ_1G;
	} else if (PAGE_SIZE == SZ_16K) {
		pgsize_bitmask = SZ_16K | SZ_32M;
	} else if (PAGE_SIZE == SZ_64K){
		pgsize_bitmask = SZ_64K | SZ_512M;
	}

	/* All page sizes that fit the size */
	pgsizes = pgsize_bitmask & GENMASK_ULL(__fls(size), 0);

	/* Address must be aligned to page size */
	if (likely(paddr))
		pgsizes &= GENMASK_ULL(__ffs(paddr), 0);

	WARN_ON(!pgsizes);

	return BIT(__fls(pgsizes));
}

static void __smmu_host_stage2_idmap(struct kvm_hyp_iommu_domain *domain,
				     phys_addr_t start, phys_addr_t end, int prot)
{
	size_t size = end - start;
	size_t pgsize, pgcount;
	size_t mapped, unmapped;
	int ret;
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct io_pgtable *pgtable = smmu_domain->pgtable;

	if (prot) {
		if (!(prot & IOMMU_MMIO))
			prot |= IOMMU_PASSTHROUGH;

		while (size) {
			mapped = 0;
			pgsize = smmu_pgsize_idmap(size, start);
			pgcount = size / pgsize;
			ret = pgtable->ops.map_pages(&pgtable->ops, start, start,
						     pgsize, pgcount, prot, 0, &mapped);
			size -= mapped;
			start += mapped;
			if (mapped && smmu_phys_is_dram(start - mapped))
				smmu_telemetry_req_idmap_map(mapped, smmu_domain->snapshot_done);

			if (!mapped || ret)
				return;
		}
	} else {
		while (size) {
			pgsize = smmu_pgsize_idmap(size, start);
			pgcount = size / pgsize;
			unmapped = pgtable->ops.unmap_pages(&pgtable->ops, start,
							    pgsize, pgcount, NULL);
			size -= unmapped;
			start += unmapped;
			if (WARN_ON(!unmapped))
				return;

			if (smmu_phys_is_dram(start - unmapped))
				smmu_telemetry_req_idmap_unmap(unmapped,
							       smmu_domain->snapshot_done);
		}
	}
}

static void smmu_host_stage2_idmap(struct kvm_hyp_iommu_domain *domain,
				   phys_addr_t start, phys_addr_t end, int prot)
{
	struct hyp_arm_smmu_v3_domain *smmu_domain = domain->priv;
	struct io_pgtable *pgtable = smmu_domain->pgtable;
	phys_addr_t block_region_start = kvm_hyp_smmu_global_config.block_region_start;
	phys_addr_t block_region_end = block_region_start +
		kvm_hyp_smmu_global_config.block_region_size;

	end = min(end, BIT(pgtable->cfg.oas));

	if (start < SZ_2M)
		start = SZ_2M;

	hyp_spin_lock(&smmu_domain->pgt_lock);
	smmu_domain->pgt_version++;
	hyp_spin_unlock(&smmu_domain->pgt_lock);

	if (start < block_region_start && end > block_region_end) {
		__smmu_host_stage2_idmap(domain, start, block_region_start, prot);
		__smmu_host_stage2_idmap(domain, block_region_end, end, prot);
		return;
	}

	if (start >= block_region_start && start < block_region_end)
		start = block_region_end;

	if (end > block_region_start && end <= block_region_end)
		end = block_region_start;

	if (start >= end)
		return;

	__smmu_host_stage2_idmap(domain, start, end, prot);
}

#ifdef MODULE
int smmu_init_hyp_module(const struct pkvm_module_ops *ops)
{
	if (!ops)
		return -EINVAL;

	mod_ops = ops;
	return 0;
}
#endif

/* Shared with the kernel driver in EL1 */
struct kvm_iommu_ops smmu_ops = {
	.init				= smmu_init,
	.get_iommu_by_id		= smmu_id_to_iommu,
	.alloc_domain			= smmu_alloc_domain,
	.free_domain			= smmu_free_domain,
	.iotlb_sync			= smmu_iotlb_sync,
	.attach_dev			= smmu_attach_dev,
	.attach_dev_nested		= smmu_attach_dev_nested,
	.detach_dev			= smmu_detach_dev,
	.detach_dev_nested		= smmu_detach_dev_nested,
	.nested_cfg_sync		= smmu_nested_cfg_sync,
	.iotlb_inv_nested_domain	= smmu_iotlb_inv_nested_domain,
	.map_pages			= smmu_map_pages,
	.unmap_pages			= smmu_unmap_pages,
	.iova_to_phys			= smmu_iova_to_phys,
	.dabt_handler			= smmu_dabt_handler,
	.suspend			= smmu_suspend,
	.resume				= smmu_resume,
	.host_stage2_idmap		= smmu_host_stage2_idmap,
	.dev_block_dma			= smmu_dev_block_dma,
	.get_iommu_token_by_id		= smmu_id_to_token,
	.iotlb_sync_map			= smmu_iotlb_sync_map,
};
