...
#endif // IS_ENABLED(CONFIG_GS_DRM_PANEL_UNIFIED)
	...
	struct device_node *np, int index)
{
	...
	/* pt1a */
	retval = of_property_read_string_index(np, "synaptics,pt1a_low_name", index, &name);
	if (retval == 0) {
		strncpy(hw_if->pt1a_low_limit_name, name, sizeof(hw_if->pt1a_low_limit_name));
	} else {
		strncpy(hw_if->pt1a_low_limit_name, "synaptics,pt1a_low",
			sizeof(hw_if->pt1a_low_limit_name));
	}
...
static int google_parse_setting_id_from_connector(struct device_node *np,
						  struct device_node *panel_np)
{
#if IS_ENABLED(CONFIG_GS_DRM_PANEL_UNIFIED)
	u32 panel_id, current_panel_id, current_setting_id;
	int i, array_len, ret;

	if (!of_property_read_bool(np, "synaptics,panel_id_map")) {
		return -EOPNOTSUPP;
	};

	struct gs_drm_connector *gs_drm_connector =
		of_gs_drm_find_connector(panel_np);
	if (gs_drm_connector == NULL) {
		LOGE("Failed to get gs_connector");
		return -EOPNOTSUPP;
	}
	...
	LOGI("Get panel id: %x", panel_id);

	array_len = of_property_count_u32_elems(np, "synaptics,panel_id_map");
	if (array_len < 0 || array_len % 2 != 0) {
		LOGE("Fail to get synaptics,panel_id_map count %d", array_len);
		return -EINVAL;
	}

	for (i = 0; i < array_len; i += 2) {
		ret = of_property_read_u32_index(np, "synaptics,panel_id_map", i,
						 &current_panel_id);
		if (ret < 0) {
			LOGE("Failed to read element at index %d\n", i);
			return ret;
		}

		if (current_panel_id == panel_id) {
			ret = of_property_read_u32_index(np, "synaptics,panel_id_map", i + 1,
							 &current_setting_id);
			if (ret < 0) {
				LOGE("Failed to read setting_id at index %d\n", i + 1);
				return ret;
			}
			LOGI("Match setting id(%d) by panel id(%x)",
			     current_setting_id, panel_id);
			return current_setting_id;
		}
	}
...
static int google_parse_panel_setting_id(struct syna_hw_interface *hw_if, struct device_node *np)
{
	int index = 0;
	int retval = 0;
	...
	for (index = 0 ;; index++) {
		...
		retval =
			google_parse_setting_id_from_connector(np, panelmap.np);
		if (retval >= 0)
			return retval;
...
static int syna_spi_probe(struct spi_device *spi)
{
	...
	if (retval < 0) {
		LOGW("Early return by syna_spi_parse_dt: %d\n", retval);
		return retval;
	}
#endif
