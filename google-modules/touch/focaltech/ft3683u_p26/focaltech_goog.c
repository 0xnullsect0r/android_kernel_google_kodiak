...
// Reference: proc_int2_write
static int gti_set_int2_mode(void *private_data, struct gti_int2_cmd *cmd)
{
    int ret = 0;
    int mode = 0;
    ...
    switch (cmd->setting) {
      case GTI_INT2_MODE_KEEP_LOW:
        mode = INT2_MODE_KEEP_LOW;
        break;
      case GTI_INT2_MODE_KEEP_HIGH:
        mode = INT2_MODE_KEEP_HIGH;
        break;
      case GTI_INT2_MODE_AUTO:
        mode = INT2_MODE_AUTO;
        break;
      default:
        FTS_ERROR("Unsupported INT2 mode");
        return -EOPNOTSUPP;
    }

    ret = fts_write_reg_safe(FTS_REG_INT2, mode);
    if (ret < 0) {
        FTS_ERROR("Failed to write to %02X, ret: %d", FTS_REG_INT2, ret);
        return ret;
    }
...
// Reference: proc_int2_read
static int gti_get_int2_mode(void *private_data, struct gti_int2_cmd *cmd)
{
    int ret = 0;
    u8 mode = 0;
    ...
    if (ret < 0) {
        FTS_ERROR("Failed to read to %02X, ret: %d", FTS_REG_INT2, ret);
        return ret;
    }

    switch (mode) {
      case INT2_MODE_KEEP_LOW:
        cmd->setting = GTI_INT2_MODE_KEEP_LOW;
        break;
      case INT2_MODE_KEEP_HIGH:
      ...
      case INT2_MODE_AUTO:
        ...
        break;
      default:
        FTS_ERROR("Unknown INT2 type: %d", mode);
        return -EINVAL;
    }
...
static int gti_get_int2_status(void *private_data, struct gti_int2_status_cmd *cmd)
{
    int ret = 0;
    int status = 0;
    u8 reg[1] = { FTS_REG_CUSTOMER_STATUS };
    struct fw_status_ts fw_status = {};

    ret = fts_read(reg, 1,  fw_status.data, sizeof(struct fw_status_ts));
    if (ret < 0) {
      FTS_ERROR("Failed to read %02X, ret: %d", FTS_REG_CUSTOMER_STATUS, ret);
      return ret;
    }

    status = fw_status.B1_b6_INT2_status;
    switch (status) {
      case INT2_LOW:
        cmd->setting = GTI_INT2_STATUS_LOW;
        break;
      case INT2_HIGH:
        cmd->setting = GTI_INT2_STATUS_HIGH;
        break;
      default:
        FTS_ERROR("Unknown INT2 status: %d", status);
        return -EINVAL;
    }
...
static int gti_get_vsync_hsync_frequency(
    void* private_data, struct gti_vsync_hsync_frequency_cmd* cmd) {
  int i = 0;
  int ret = 0;
  u8 hsync_reg[1] = {FTS_REG_HSYNC_FREQ};
  u8 vsync_reg[1] = {FTS_REG_VSYNC_FREQ};
  u8 detect_hsync_vsync_reg[1] = {FTS_REG_DETECT_HSYNC_VSYNC};
  u16 hsync_freq_khz = 0;
  u16 vsync_freq_hz = 0;
  u16 detect_hsync_vsync_result = 0;

  /* Initiate vsync and hsync detection */
  ret = fts_write_reg_safe(FTS_REG_DETECT_HSYNC_VSYNC, 1);
  if (ret < 0) {
    FTS_ERROR("Failed to write 1 to 0xCC, ret: %d", ret);
    return ret;
  }

  msleep(50);

  /* Wait until firmware has finished by checking if 0xCC == 0 */
  for (i = 0; i < 3; i++) {
    ret = fts_read(detect_hsync_vsync_reg, 1,
                   (unsigned char*)&detect_hsync_vsync_result, sizeof(u16));
    if (ret < 0) {
      FTS_ERROR("Failed to read 0xCC, ret: %d", ret);
      msleep(5);
      continue;
    }

    if (detect_hsync_vsync_result == 0) {
      break;
    } else {
      FTS_INFO("Retry count: %d, 0xCC: %02X", i, detect_hsync_vsync_result);
      msleep(5);
      continue;
    }
  }
  ...
  /* Abort if timeout */
  if (detect_hsync_vsync_result != 0) {
    FTS_ERROR("0xCC does not equal to 0, 0xCC: %02X",
              detect_hsync_vsync_result);
    return -ETIME;
  }

  /* Read hsync frequency in kHz */
  ret = fts_read(hsync_reg, 1, (unsigned char*)&hsync_freq_khz, sizeof(u16));
  if (ret < 0) {
    FTS_ERROR("Failed to read 0xCA, ret: %d", ret);
    return ret;
  }

  /* Read vsync frequency in Hz */
  ret = fts_read(vsync_reg, 1, (unsigned char*)&vsync_freq_hz, sizeof(u16));
  if (ret < 0) {
    FTS_ERROR("Failed to read 0xCB, ret: %d", ret);
    return ret;
  }

  /* Need to divide by 10 because the last digit is a decimal point. Ideally,
   * this should be a float but currently GTI only supports u16 */
  cmd->hsync_value = endian_swap(hsync_freq_khz) / 10;
  cmd->vsync_value = endian_swap(vsync_freq_hz) / 10;
  return ret;
}
    ...
    struct fts_ts_data *ts)
{
    ...
    options->get_int2_mode = gti_get_int2_mode;
    options->get_int2_status = gti_get_int2_status;
    ...
    options->get_vsync_hsync_frequency = gti_get_vsync_hsync_frequency;
    ...
    options->set_int2_mode = gti_set_int2_mode;
