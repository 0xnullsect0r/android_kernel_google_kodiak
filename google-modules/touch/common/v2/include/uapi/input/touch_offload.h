...
#define TOUCH_OFFLOAD_INTERFACE_MINOR_VERSION 2
...
#define CONTEXT_CHANNEL_TYPE_VENDOR_REGISTER  0x0400

/* Inclusive range of valid context channels */
#define CONTEXT_CHANNEL_BIT_START 0x0100
#define CONTEXT_CHANNEL_BIT_END   0x0400
...
#define TOUCH_OFFLOAD_VENDOR_REGISTER_DATA_SIZE 128
/* TouchOffloadDataVendorRegister
 *
 * header - header shared by all channels in a frame
 * valid_size - valid size for data array
 * data - pointer to vendor register buffer
 */
struct TouchOffloadVendorRegister {
	struct TouchOffloadChannelHeader header;
	__u32 valid_size;
	__u8 reserved[16];
	__u8 data[TOUCH_OFFLOAD_VENDOR_REGISTER_DATA_SIZE];
} __attribute__((packed));

#define TOUCH_OFFLOAD_FRAME_SIZE_VENDOR_REGISTER \
		(sizeof(struct TouchOffloadVendorRegister))
