...
#if IS_ENABLED(CONFIG_SOC_MBU)
		google_pcie_dump_debug(plat_priv->rc_num);
#endif
...
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
		if (!test_bit(CNSS_PCI_PROBE_DONE, &plat_priv->driver_state)) {
			cnss_pr_err("Don't register driver as PCI probe not complete\n");
			if (cancel_delayed_work_sync(&plat_priv->wlan_reg_driver_work))
				cnss_pr_info("Canceled (pending or running) wlan_reg_driver_work\n");
			goto out;
		}
#endif
...
static int cnss_master_bind(struct device *dev)
{
	struct cnss_plat_data *plat_priv = dev_get_drvdata(dev);
	int ret = 0;

	cnss_pr_info("WCN Power Controller is ready, starting cnss initialization\n");
...
static void cnss_master_unbind(struct device *dev)
{
	cnss_pr_info("WCN Power Controller is unbinding\n");
}

static const struct component_master_ops cnss_master_ops = {
	.bind = cnss_master_bind,
	.unbind = cnss_master_unbind,
};
#endif
...
static int cnss_probe(struct platform_device *plat_dev)
{
	...
	cnss_google_plat_probe(plat_dev);
#endif
	...
	if (!pwr_node) {
		cnss_pr_err("Failed to find wcn-power-controller phandle\n");
		return -ENODEV;
	}

	component_match_add(&plat_dev->dev, &match, cnss_component_compare, pwr_node);
	of_node_put(pwr_node);

	return component_master_add_with_match(&plat_dev->dev, &cnss_master_ops, match);
#else
	/* Make sure all platform related init are done before
	 * device power on and bus init.
	 */
...
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
	component_master_del(&plat_dev->dev, &cnss_master_ops);
#endif
