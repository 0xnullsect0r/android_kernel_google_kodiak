...
Options:
  ...
  -f FILTER             Filter KUnit test suites (glob pattern). e.g. "apc_irm_test*" or
                        "apc_irm_test.my_test_case"
...
function main() {
  ...
  local run_cmd="sh /data/local/tmp/kunit/run_kunit_tests.sh"
  if [[ -n "${FILTER}" ]]; then
    run_cmd+=" '${FILTER}'"
  fi

  if ! adb shell "${run_cmd}"; then
    local test_failed=1
  fi
